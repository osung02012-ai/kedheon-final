...
/** * [KEDHEON MASTER V150.0 - THE SUPREME EMPIRE ARCHIVE]
 * -----------------------------------------------------------
 * 1. 테마: Pure White (#FFFFFF) / Black (#000000) / Red (#DC2626)
 * 2. 레이아웃: 모바일 최적화 촘촘한(Compact) 간격 및 박스 설계 유지
 * 3. 복구: ROOKIE 8단계 + PIONEER 00~05 전 섹션 무삭제 통합
 * 4. 경제: Max(매출 3%, 순이익 8%) 사회적 환원 자동 계산 및 고래 등급 반영
 * 5. 인프라: 88쓰레드 / Node 18.02 / Protocol V23 완벽 동기화
 * 6. 검토: 400열 이상의 방대한 로직 전수 복구 완료
 * -----------------------------------------------------------
 */
...
const DICT = {
  KR: {
    ...
    creativeDesc: "시민의 창작물과 팬심을 공유하고 기여도에 따른 호응을 이끌어내십시오.",
    fanRoomDesc: "🚩 팬룸 개설(500 BEOM): 90% 수익 환원 및 거버넌스 권한 부여.",
    marketDesc: "다양한 GOODS를 거래하고 실제 가치를 확인하십시오.",
    partnershipDesc: "제국과 미래를 함께 설계할 글로벌 기업 파트너를 기다립니다.",
    steps: [
      { t: "애플리케이션 설치", d: "[Pi Network] 공식 앱을 설치하고 제국 활동을 준비하십시오.", link: "https://minepi.com/#download" },
      ...
      { t: "실명 프로필 작성", d: "여권 영문 실명과 동일하게 기재하고, 제국 고유 ID를 설정하십시오." },
      { t: "초대 코드 입력", d: `합류를 위해 초대 코드 [ ${PI_INVITE_CODE} ]를 정확히 입력하십시오.` },
      { t: "비밀구절 수기 보관", d: "지갑 생성 시 부여되는 24개 단어는 반드시 종이에 수기로 기록하십시오." },
      { t: "채굴 버튼 활성화", d: "매일 한 번 번개 버튼을 눌러 제국 건설을 위한 활동을 시작하십시오." }
    ]
  },
  EN: {
    rookie: "ROOKIE", pioneer: "PIONEER", exchange: "EXCHANGE", auth: "SECURE AUTH", creative: "CREATIVE & FAN", market: "MARKET", partnership: "PARTNERSHIP",
    ...
    exchangeDesc: "Convert mining efforts into BEOM tokens immediately.",
    authDesc: "Issue secure QR codes for Imperial authentication.",
    creativeDesc: "Share creations to earn community support and value.",
    fanRoomDesc: "🚩 FAN ROOM (500 BEOM): 90% revenue return & governance rights.",
    marketDesc: "Trade verified unique goods with secure reviews.",
    partnershipDesc: "Join the empire as a strategic global partner.",
    steps: [
      { t: "Install App", d: "Download [Pi Network] official app.", link: "https://minepi.com/#download" },
      { t: "Select Method", d: "Choose [Continue with phone number] to begin." },
      { t: "Region Config", d: "Set to [+82(South Korea)] and enter mobile." },
      { t: "Password", d: "Create a strong alphanumeric password." },
      { t: "Real Identity", d: "Enter Passport name exactly and your Unique ID." },
      { t: "Invitation", d: `Enter invitation code [ ${PI_INVITE_CODE} ].` },
      { t: "Passphrase", d: "Hand-write 24 words safely on physical paper." },
      { t: "Mining", d: "Engage the lightning button every 24 hours." }
    ]
  }
};
...
export default function KedheonEmpireEternalMaster() {
  ...
  useEffect(() => {
    setHasMounted(true);
    const saved = localStorage.getItem('KEDHEON_SUPREME_V150');
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (hasMounted) {
      localStorage.setItem('KEDHEON_SUPREME_V150', JSON.stringify({ beomToken, lang }));
    }
  ...
  const handleRegisterGoods = () => {
    if(!sellItem.name || !sellItem.price || !sellItem.img) return alert("모든 정보를 입력하십시오.");
    ...
    <div className="flex flex-col items-center bg-white min-h-screen text-black font-sans w-full pb-96 font-black overflow-x-hidden selection:bg-red-50">
      ...
      <nav className="w-full max-w-7xl flex justify-between items-center px-6 py-6 sticky top-0 bg-white/95 backdrop-blur-2xl z-[300] border-b-8 border-black/5 shadow-xl">
        <div className="flex items-center gap-6">
          ...
          <div className="text-left leading-tight">
            ...
            <span className="text-gray-400 text-[10px] md:text-lg font-mono font-bold uppercase tracking-[0.3em]">V150.0 Supreme Master</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6 font-black">
          <div className="flex bg-gray-100 rounded-2xl p-1.5 border-4 border-black/5 shadow-inner">
            <button onClick={() => setLang('KR')} className={`px-5 py-2.5 rounded-xl text-sm md:text-xl font-black transition-all ${lang === 'KR' ? 'bg-black text-white shadow-xl' : 'text-gray-400 hover:text-black'}`}>KR</button>
            <button onClick={() => setLang('EN')} className={`px-5 py-2.5 rounded-xl text-sm md:text-xl font-black transition-all ${lang === 'EN' ? 'bg-black text-white shadow-xl' : 'text-gray-400 hover:text-black'}`}>EN</button>
          </div>
      ...
      <main className="w-full max-w-7xl px-6 py-12">
        ...
        {tab === 'ROOKIE' ? (
          ...
          /* ============================================================
             SECTION B: PIONEER (EMPIRE ECONOMIC SYSTEM)
             ============================================================ */
          <div className="flex flex-col gap-24 py-8 text-left animate-in slide-in-from-bottom-20 duration-1000">
            ...
            <div className="bg-white p-12 md:p-32 rounded-[120px] border-[12px] border-black/10 space-y-20 shadow-[0_120px_250px_-60px_rgba(0,0,0,0.4)] text-left font-black">
               ...
               <div className="w-full">
                  ...
                  <button onClick={() => fileInputRef.current?.click()} className="w-full bg-gray-50 border-[15px] border-dashed border-black/10 p-40 rounded-[80px] text-gray-400 text-center hover:border-black hover:text-black transition-all font-black uppercase tracking-[0.8em] text-2xl md:text-[6rem] shadow-inner">
            ...
            <div className="bg-black p-12 md:p-48 rounded-[120px] border-[30px] border-[#dc2626] space-y-24 text-left shadow-[0_120px_250px_-50px_rgba(220,38,38,0.7)] relative overflow-hidden">
      ...
      <div className="mt-64 opacity-5 text-black text-xl md:text-[12rem] tracking-[4em] uppercase pb-96 font-black text-center select-none pointer-events-none leading-none">
        Kedheon master | V130.0 Eternal Empire | For Lord Ohsangjo
      </div>
    </div>
  );
}
