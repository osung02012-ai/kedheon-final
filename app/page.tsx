...
export default function KedheonPortal() {
  ...
  // --- [2. 비주얼 에셋 (이미지 누락 방지)] ---
  const charImage = "https://images.unsplash.com/photo-1561037404-61cd46aa615b?auto=format&fit=crop&q=80&w=300"; // 호랑이 상징
  ...
  const registerAsset = () => {
    ...
    alert(isAdRequest ? "제국 공식 광고가 전 구역에 게재되었습니다!" : "제국 자산 등록이 완료되었습니다.");
  };
      ...
      <div className="w-full max-w-2xl">
        {tab === 'ROOKIE' ? (
          ...
          <div className="flex flex-col items-center text-center py-20 animate-in fade-in zoom-in duration-500">
            <img src={charImage} className="w-48 h-48 rounded-full border-4 border-[#daa520] object-cover mb-8 shadow-[0_0_50px_rgba(218,165,32,0.2)]" alt="Imperial Tiger" />
          ...
          <div className="flex flex-col gap-8 animate-in slide-in-from-bottom-10 duration-700">
            ...
            {/* 3. 제국 인증 QR 시스템 (결제형) */}
            <div className="bg-[#111] p-8 rounded-3xl border border-[#daa520]/20 text-center relative">
              <div className="absolute top-3 right-5 text-[10px] text-gray-600 font-mono">Fee: {QR_PURCHASE_COST} BEOM</div>
              ...
              <div className="relative w-full max-w-[220px] mx-auto aspect-square rounded-3xl border border-white/10 overflow-hidden shadow-2xl">
                ...
                <div className={`absolute inset-0 flex flex-col items-center justify-center p-6 transition-all duration-500 ${isQrActive ? 'bg-black/10' : 'bg-black/95 backdrop-blur-2xl'}`}>
                  {isQrActive ? (
                    <div className="bg-white p-3 rounded-2xl shadow-white/20 shadow-2xl animate-in zoom-in-50 duration-300">
              ...
              {qrType === 'BUSINESS' && (
                <div className="mt-8 flex flex-col gap-3 max-w-[250px] mx-auto">
                  <input type="text" value={businessName} onChange={(e) => {setBusinessName(e.target.value); setIsQrActive(false);}} placeholder="기업/브랜드 명칭" className="bg-black/50 border border-white/10 p-3.5 rounded-xl text-sm focus:border-[#daa520] outline-none text-center" />
                  <input type="text" value={businessID} onChange={(e) => {setBusinessID(e.target.value); setIsQrActive(false);}} placeholder="사업자/고유 ID" className="bg-black/50 border border-white/10 p-3.5 rounded-xl text-sm focus:border-[#daa520] outline-none text-center font-mono" />
                </div>
              )}
            </div>
                ...
                {ecosystemFullApps.map(app => (
                  <button key={app} onClick={() => handleHubClick(app)} className="group relative">
                    <div className="h-20 bg-white/5 rounded-2xl border border-white/10 flex flex-col items-center justify-center p-3 hover:bg-[#daa520] hover:border-transparent transition-all duration-300 group-active:scale-95">
                      <span className="text-[10px] font-black text-[#daa520] group-hover:text-black text-center leading-tight">{app}</span>
            ...
            <div className="bg-[#111] p-8 rounded-3xl border border-white/5 relative">
              ...
              <div className="flex flex-col gap-4">
                ...
                <input type="text" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="콘텐츠 또는 광고 제목" className="bg-black p-4 rounded-2xl border border-white/10 text-sm focus:border-[#daa520] outline-none transition-all" />
                <textarea value={newDesc} onChange={(e) => setNewDesc(e.target.value)} placeholder="상세 설명 (공고 및 홍보 문구)" className="bg-black p-4 rounded-2xl border border-white/10 text-sm h-24 outline-none focus:border-[#daa520] resize-none transition-all" />
                  ...
                  assets.filter(a => category === 'ALL' || a.category === category).map((a) => (
                    <div key={a.id} className={`p-6 rounded-3xl border-l-8 transition-all animate-in slide-in-from-right-10 duration-500 ${a.isAd ? 'bg-[#daa520]/10 border-[#daa520] shadow-[0_10px_20px_rgba(218,165,32,0.1)]' : 'bg-white/5 border-gray-800'}`}>
                      ...
                      <div className="flex justify-between items-center pt-3 border-t border-white/5">
                        <span className="text-[#daa520] text-[9px] font-black uppercase tracking-widest bg-[#daa520]/10 px-2 py-0.5 rounded-md">{a.category}</span>
                        <span className="text-[9px] text-gray-600 font-mono italic">Created by {a.owner}</span>
                      </div>
                    </div>
                  ))
                ) : (
                  <p className="text-center py-10 text-gray-700 text-[10px] uppercase font-bold tracking-widest">현재 영토에 등록된 자산이 없습니다</p>
                )}
              </div>
            </div>
            ...
            <div className="text-center py-16 opacity-20">
               ...
               <p className="text-[8px] font-mono tracking-[0.8em] uppercase">Kedheon Empire Digital Infrastructure | Powered by 88-Threads Engine</p>
            </div>
