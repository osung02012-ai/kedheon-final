...
export default function KedheonPortal() {
  ...
  useEffect(() => {
    setHasMounted(true);
    const saved = localStorage.getItem('kedheon_v55_master');
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (hasMounted) {
      localStorage.setItem('kedheon_v55_master', JSON.stringify({ token: beomToken, assets }));
    }
  ...
  const postContent = () => {
    ...
    alert(`${postType === 'CREATION' ? '창작물' : '방송'} 등록 완료`);
  };

  const promoteAsset = (id: number) => {
    if (beomToken < 50) return alert("BEOM 토큰 부족");
    setBeomToken(p => p - 50);
  ...
  return (
    <div className="flex flex-col items-center bg-black min-h-screen text-white font-sans w-full pb-20 overflow-x-hidden text-center">
      ...
      <div className="w-full max-w-5xl px-4 md:px-6">
        ...
        {tab === 'ROOKIE' ? (
          ...
          <div className="flex flex-col items-center py-16 animate-in fade-in duration-700">
            ...
            {!showOnboarding ? (
              <button onClick={() => setShowOnboarding(true)} className="bg-white text-black px-12 py-5 rounded-full font-black text-xl md:text-2xl border-4 border-[#daa520] hover:scale-105 transition-all shadow-2xl">시민권 획득 가이드</button>
            ) : (
              <div className="w-full bg-[#111] p-6 md:p-10 rounded-[40px] border-4 border-white shadow-2xl space-y-8 animate-in slide-in-from-bottom-5">
                <h2 className="text-[#daa520] font-black text-2xl md:text-4xl uppercase italic tracking-widest">Pioneer Onboarding</h2>
                ...
                <div className="flex flex-col md:flex-row gap-4 pt-4">
                  <button onClick={() => window.open(PI_APP_STORE_URL)} className="flex-1 bg-white text-black py-6 rounded-2xl border-4 border-[#daa520] hover:scale-105 uppercase transition-all font-black">App Store</button>
                  <button onClick={() => window.open(PI_PLAY_STORE_URL)} className="flex-1 bg-white text-black py-6 rounded-2xl border-4 border-[#daa520] hover:scale-105 uppercase transition-all font-black">Play Store</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-12 py-10 animate-in slide-in-from-bottom-5">
            ...
            <div className="w-full bg-[#111] p-8 md:p-12 rounded-[50px] border-4 border-[#daa520] shadow-2xl">
              <h2 className="text-[#daa520] font-black text-3xl md:text-6xl uppercase italic tracking-tighter mb-6 font-sans">
                누리고 즐기고 선점하세요
              </h2>
              <div className="space-y-4 text-white/90 text-lg md:text-2xl font-bold leading-relaxed tracking-tight">
                ...
                <p>한정된 디지털 자산과 기여 보상을 남보다 먼저 <span className="text-[#daa520]">선점</span>하여 주권을 확보합니다.</p>
              </div>
            </div>
            ...
            <div className="flex flex-col w-full">
              ...
              <div className="bg-[#111] p-8 rounded-[40px] border-4 border-white shadow-2xl grid md:grid-cols-2 gap-8 items-center">
                <div className="text-left space-y-4 font-black">
                  ...
                  <p className="text-white/60 text-sm md:text-lg italic font-bold">1 Pi = 314.159 BEOM | 공식 환전 시스템</p>
                </div>
                <button onClick={() => setBeomToken(p => p + 314.15)} className="w-full bg-[#daa520] text-black py-6 rounded-2xl font-black text-xl md:text-2xl border-4 border-white shadow-xl hover:scale-105 active:scale-95 transition-all">EXCHANGE PI TO BEOM</button>
              </div>
            </div>

            {/* 02. 🌐 보안 인증 및 고유 QR 디자인 */}
            <div className="flex flex-col w-full">
              ...
              <div className="bg-[#111] p-6 md:p-10 rounded-[40px] border-4 border-white flex flex-col items-center gap-8 shadow-2xl">
                ...
                {/* [중요] QR 고유 이미지 디자인 노출 로직 */}
                <div className={`relative p-2 md:p-4 bg-black border-4 rounded-[50px] shadow-[0_0_80px_rgba(218,165,32,0.5)] transition-all flex items-center justify-center overflow-hidden ${isQrActive ? 'border-[#daa520]' : 'opacity-10 border-white'}`}>
                  {isQrActive ? (
                    <img 
                      src={qrType === 'PERSONAL' ? "/qr-personal.png" : "/qr-business.png"} 
                      className="w-full max-w-[500px] h-auto rounded-3xl" 
                      alt="SECURE_QR_DESIGN" 
                    />
                  ) : (
                    <div className="w-64 h-64 md:w-96 md:h-96 flex items-center justify-center text-white/5 text-4xl md:text-8xl font-black italic uppercase font-black">Secure</div>
                  )}
                </div>
            ...
            <div className="flex flex-col w-full text-left">
              ...
              <div className="w-full bg-[#111] p-6 md:p-10 rounded-[40px] border-4 border-[#daa520] space-y-6 mb-12 shadow-xl">
                ...
                <textarea value={newDesc} onChange={(e) => setNewDesc(e.target.value)} placeholder="상세 내용을 입력하십시오" className="bg-black border-4 border-white p-4 rounded-xl w-full text-sm md:text-lg font-bold text-white/80 h-32 resize-none outline-none focus:border-[#daa520]" />
                <button onClick={postContent} className="w-full py-5 rounded-2xl bg-[#daa520] text-black font-black text-xl md:text-2xl border-4 border-white shadow-lg active:scale-95 transition-all uppercase font-black">제국 피드 등록하기 (10 BEOM)</button>
              </div>
              ...
              <button onClick={() => setShowCreateModal(true)} className="w-full py-6 rounded-3xl bg-white text-black font-black text-2xl border-4 border-[#daa520] mb-12 shadow-xl hover:bg-[#daa520] transition-all uppercase font-black italic">➕ 팬방 개설 (500 BEOM)</button>
                  ...
                  <div key={a.id} className={`bg-[#111] rounded-[50px] border-4 p-8 md:p-12 space-y-8 shadow-2xl relative transition-all ${a.isPromoted ? 'border-[#daa520] ring-8 ring-[#daa520]/10' : 'border-white hover:border-[#daa520]'}`}>
                    <div className="flex justify-between items-start font-black">
                      <div className="space-y-3">
                        <span className="bg-white text-black px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter">{a.type}</span>
                    ...
                    <div className="pt-8 border-t-4 border-white/10 flex flex-wrap justify-between items-center gap-4 font-black">
                      <div className="flex gap-2">
                        ...
                        {!a.isPromoted && (
                          <button onClick={() => promoteAsset(a.id)} className="bg-black text-[#daa520] px-8 py-4 rounded-2xl font-black text-lg border-4 border-[#daa520] active:scale-95 transition-all shadow-lg">광고하기 (50 BEOM)</button>
                        )}
                      </div>
      ...
      <div className="mt-20 opacity-40 text-center w-full pb-10 font-mono text-white text-[10px] tracking-[1em] uppercase font-black">
        KEDHEON EMPIRE | INTEGRITY MASTER V55.0 | ohsangjo
      </div>
    </div>
  );
}
