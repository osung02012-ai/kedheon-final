...
/** * [KEDHEON MASTER V108.6 - THE TRUE FULL ARCHIVE]
 * -----------------------------------------------------------
 * 1. 테마: Pure White (#FFFFFF) / Black (#000000) / Red (#DC2626)
 * 2. 전수복구: ROOKIE(8단계) + PIONEER(01~05 전 섹션) 무삭제 통합
...
export default function KedheonEmpireFullMaster() {
  ...
  useEffect(() => {
    setHasMounted(true);
    const saved = localStorage.getItem('KEDHEON_V108_6');
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (hasMounted) {
      localStorage.setItem('KEDHEON_V108_6', JSON.stringify({ beomToken, lang, fanRooms }));
    }
  ...
  return (
    <div className="flex flex-col items-center bg-white min-h-screen text-black font-sans w-full pb-64 font-black overflow-x-hidden selection:bg-red-50">
      ...
      <nav className="w-full max-w-7xl flex justify-between items-center px-3 py-2 sticky top-0 bg-white/95 backdrop-blur-md z-[250] border-b-2 border-black/5">
        <div className="flex items-center gap-2">
          ...
          <div className="text-left leading-tight">
            ...
            <span className="text-gray-400 text-[7px] font-mono font-bold uppercase">V108.6 MASTER</span>
          </div>
        </div>
      ...
      <main className="w-full max-w-4xl px-3 py-4">
        ...
        {tab === 'ROOKIE' ? (
          <div className="flex flex-col gap-4 text-left animate-in fade-in">
            <div className="flex flex-col items-center text-center gap-3 py-8 bg-gray-50 rounded-[30px] border border-black/5 relative shadow-inner">
              <img src="/kedheon-character.png" className="w-24 h-24 md:w-32 md:h-32 rounded-2xl border-2 border-black shadow-lg" alt="M" />
              <div className="px-2">
                <h1 className="text-black text-xl md:text-3xl uppercase mb-0.5 font-black tracking-tighter">{L.invitation}</h1>
                <p className="text-[#dc2626] text-sm md:text-xl uppercase tracking-widest border-b-2 border-[#dc2626] pb-0.5 inline-block font-black italic">{L.procedure}</p>
              </div>
            </div>
              ...
              {L.steps.map((step, i) => (
                <div key={i} className={`p-4 bg-white rounded-xl border-2 flex items-center gap-4 transition-all ${i === 0 ? 'border-[#dc2626] bg-red-50/5 shadow-md scale-[1.01]' : 'border-black/5 opacity-80'}`}>
                  ...
                  <div className="flex-1">
                    ...
                    {/* [FIX] 앱 다운로드 공식 링크 버튼 강화 */}
                    {step.link && (
                      <button onClick={() => window.open(step.link, '_blank')} className="mt-3 bg-[#dc2626] text-white px-6 py-2.5 rounded-xl text-[10px] md:text-sm font-black uppercase hover:bg-black transition-all shadow-lg flex items-center gap-2 animate-bounce">
                        ↓ {L.download}
                      </button>
                    )}
                  </div>
            ...
            <div className="p-5 bg-black text-white rounded-2xl text-center shadow-xl border-2 border-black">
              <p className="text-[10px] md:text-sm font-black italic uppercase tracking-widest">초대 코드(Invitation Code): <span className="text-[#dc2626] text-xl md:text-3xl ml-2 tracking-widest">{PI_INVITE_CODE}</span></p>
            </div>
          </div>
        ) : (
          /* -- PIONEER TAB CONTENT -- */
          <div className="flex flex-col gap-8 py-1 text-left animate-in slide-in-from-bottom-2">
            ...
            {/* 00. ASSETS SECTION */}
            <div className="bg-gray-50 p-4 md:p-10 rounded-[35px] border-2 border-black shadow-xl flex flex-col md:flex-row justify-between items-center relative overflow-hidden group">
                <div className="text-left z-10 space-y-2 w-full md:w-auto font-black">
                  ...
                  <div className="flex flex-wrap gap-2 pt-1 font-black">
                    <div className="bg-black text-white px-3 py-1.5 rounded-lg text-[9px] md:text-lg font-mono font-bold border border-white/10">Index: {currentBeomValue.toFixed(2)} / PI</div>
                    <div className="bg-[#dc2626] text-white px-3 py-1.5 rounded-lg text-[9px] md:text-lg font-mono font-bold animate-pulse shadow-md border border-white/20">Redistribution: {redistributionAmount.toLocaleString()}</div>
                  </div>
                </div>
                <img src="/beom-token.png" className="w-16 h-16 md:w-44 md:h-44 object-contain group-hover:rotate-12 transition-transform mt-2 md:mt-0" alt="B" />
            </div>
            ...
            <div className="bg-white p-4 md:p-10 rounded-[25px] border-2 border-black flex justify-between items-center shadow-lg gap-4">
              ...
              <button onClick={() => {setBeomToken(p=>p+100); setTotalRevenue(p=>p+100); alert("Exchange Success");}} className="bg-black text-white px-6 py-3 md:px-12 md:py-6 rounded-2xl text-[10px] md:text-xl border-2 border-black active:scale-95 uppercase font-black shadow-xl hover:bg-[#dc2626]">
                {L.convert}
              </button>
            </div>
            ...
            <div className="bg-gray-50 p-4 md:p-12 rounded-[30px] border border-black/5 flex flex-col items-center gap-4 shadow-inner">
              ...
              {qrType === 'BUSINESS' && (
                 <input 
                   ...
                   className="w-full max-w-xs bg-white border-2 border-black p-3 rounded-xl text-center text-black text-sm font-black outline-none focus:border-[#dc2626] shadow-sm" 
                 />
              )}

              <div className={`relative bg-white border-4 rounded-[20px] flex items-center justify-center transition-all duration-500 shadow-xl ${qrType === 'PERSONAL' ? 'w-40 h-40 md:w-80 md:h-80' : 'w-full max-w-2xl aspect-video'} ${isQrActive ? 'border-[#dc2626] opacity-100' : 'opacity-20 border-black/10'}`}>
                {isQrActive ? (
                  <div className="flex flex-col items-center p-4">
                    <img src={qrType === 'PERSONAL' ? "/qr-personal.png" : "/qr-business.png"} className="h-full object-contain" alt="QR" />
                    {qrType === 'BUSINESS' && <p className="text-black text-[10px] md:text-sm font-black italic mt-2">{bizName || "ENTERPRISE"}</p>}
                  </div>
                ) : <p className="text-black text-lg md:text-6xl font-black italic tracking-widest animate-pulse uppercase">Locked</p>}
              </div>
              <button onClick={() => {if(beomToken < 50) return alert("Beom Low"); setBeomToken(p=>p-50); setIsQrActive(true);}} className="w-full max-w-md bg-black text-white py-4 md:py-8 rounded-2xl text-[10px] md:text-xl border-2 border-black active:scale-95 uppercase font-black shadow-xl">
                {L.activate} (50 BEOM)
              </button>
            </div>
            ...
            <div className="bg-white p-4 md:p-10 rounded-[30px] border-2 border-black/10 space-y-4 text-left shadow-lg font-black">
              ...
              <div className="space-y-3 font-black">
                 ...
                 <div className="flex gap-2 overflow-x-auto pb-1 border-t-2 border-gray-50 pt-3">
                   ...
                   <button onClick={() => {const n = prompt("Room Name?"); if(n) setFanRooms(p=>[...p, n]);}} className="px-4 py-1.5 rounded-full text-[9px] md:text-sm font-black border-2 border-dashed border-gray-300 text-gray-400">+</button>
                 </div>
              </div>
              
              <div className="space-y-2 font-black">
                <input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="제목 또는 팬심 공유" className="w-full bg-gray-50 border-2 border-black/5 p-4 rounded-xl text-sm md:text-xl text-black outline-none focus:border-black font-black shadow-sm" />
                <input value={newUrl} onChange={(e) => setNewUrl(e.target.value)} placeholder="이미지/영상 링크" className="w-full bg-gray-50 border-2 border-black/5 p-4 rounded-xl text-[10px] md:text-lg text-[#dc2626] outline-none" />
                <textarea value={newDesc} onChange={(e) => setNewDesc(e.target.value)} placeholder="상세 내용을 기록하십시오" className="w-full bg-gray-50 border-2 border-black/5 p-4 rounded-xl text-[10px] md:text-base text-black h-32 outline-none focus:border-black font-bold" />
              </div>
              
              <div className="flex gap-2 font-black">
                  <button onClick={() => {if(!newTitle) return; setBeomToken(p=>p-10); alert("OK"); setNewTitle(''); setNewDesc('');}} className="flex-[2] bg-black text-white py-4 md:py-8 rounded-xl text-sm md:text-2xl border-2 border-black active:scale-95 uppercase font-black shadow-xl">{L.post} (10 BEOM)</button>
                  <button onClick={() => {const n = prompt("Name?"); if(n) setFanRooms(p=>[...p, n]);}} className="flex-1 bg-white text-black py-4 md:py-8 rounded-xl text-[10px] md:text-lg border-2 border-black uppercase font-black shadow-md">🚩 CREATE</button>
              </div>
              <p className="text-gray-400 text-[8px] md:text-sm font-bold bg-gray-50 p-4 rounded-xl italic border-l-8 border-[#dc2626] leading-snug">※ {L.fanRoomDesc}</p>
            </div>
            ...
            <div className="bg-white p-4 md:p-10 rounded-[30px] border-2 border-black/10 space-y-4 shadow-xl text-left font-black">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-2 font-black">
                  <input value={sellName} onChange={(e) => setSellName(e.target.value)} placeholder="PRODUCT NAME" className="w-full bg-gray-50 border-2 border-black/5 p-4 rounded-xl text-xs md:text-lg font-black outline-none focus:border-black" />
                  <input type="number" value={sellPrice} onChange={(e) => setSellPrice(e.target.value)} placeholder="PRICE (BEOM)" className="w-full bg-gray-50 border-2 border-black/5 p-4 rounded-xl text-xs md:text-lg font-black text-[#dc2626] outline-none" />
               </div>
               <textarea value={sellDesc} onChange={(e) => setSellDesc(e.target.value)} placeholder="PRODUCT DESCRIPTION" className="w-full bg-gray-50 border-2 border-black/5 p-4 rounded-xl text-[10px] md:text-base font-bold h-24 outline-none focus:border-black" />
               <div className="w-full font-black">
                  <input type="file" accept="image/*" onChange={handleImageUpload} ref={fileInputRef} className="hidden" />
                  <button onClick={() => fileInputRef.current?.click()} className="w-full bg-gray-50 border-2 border-dashed border-black/10 p-10 rounded-xl text-gray-400 text-center hover:border-black transition-all font-black uppercase tracking-widest">
                    {sellImg ? <img src={sellImg} className="h-24 mx-auto rounded-xl border-2 border-black" alt="P" /> : "📸 UPLOAD PRODUCT PHOTO"}
                  </button>
               </div>
               <button onClick={() => {if(!sellName||!sellPrice||!sellImg) return alert("Fill Fields"); setGoods([{id:Date.now(), name:sellName, price:Number(sellPrice), img:sellImg, desc:sellDesc, reviews:[]}, ...goods]); setSellName(''); setSellImg(''); alert("Registered Successfully");}} className="w-full bg-black text-white py-4 md:py-8 rounded-xl text-sm md:text-2xl border-2 border-black active:scale-95 uppercase font-black shadow-xl">{L.register} (20 BEOM)</button>
               
               <div className="grid grid-cols-2 gap-3 pt-6 font-black">
                  {goods.map(g => (
            ...
            <div className="bg-black p-4 md:p-12 rounded-[40px] border-4 border-[#dc2626] text-left shadow-2xl relative space-y-6 overflow-hidden">
                ...
                <h3 className="text-white text-xl md:text-4xl font-black italic border-l-8 border-[#dc2626] pl-4 z-10 relative uppercase font-black">Business Portal</h3>
                <div className="space-y-3 relative z-10 font-black">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div className="space-y-1">
                            ...
                            <input value={partnerCorp} onChange={(e)=>setPartnerCorp(e.target.value.toUpperCase())} placeholder="예: Pi Global Trade" className="w-full bg-white/5 border-2 border-white/10 p-4 rounded-xl text-white outline-none focus:border-[#dc2626] font-black text-sm md:text-xl" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-[#dc2626] text-[8px] md:text-xs font-black uppercase ml-2">Contact Info</label>
                            <input value={partnerContact} onChange={(e)=>setPartnerContact(e.target.value)} placeholder="contact@email.com" className="w-full bg-white/5 border-2 border-white/10 p-4 rounded-xl text-white outline-none focus:border-[#dc2626] font-black text-sm md:text-xl" />
                        </div>
                    </div>
                    <div className="space-y-1">
                        <label className="text-[#dc2626] text-[8px] md:text-xs font-black uppercase ml-2">Proposal Details</label>
                        <textarea value={partnerMsg} onChange={(e)=>setPartnerMsg(e.target.value)} placeholder="제국과 함께할 미래 비즈니스 모델을 제안하십시오." className="w-full bg-white/5 border-2 border-white/10 p-4 rounded-xl text-white h-40 outline-none focus:border-[#dc2626] font-bold" />
                    </div>
                </div>
                <button onClick={() => {if(!partnerCorp || !partnerMsg) return alert("Fill Fields"); alert("Proposal Submitted."); setPartnerCorp(''); setPartnerContact(''); setPartnerMsg('');}} className="w-full bg-[#dc2626] text-white py-5 md:py-10 rounded-2xl text-sm md:text-3xl font-black hover:bg-white hover:text-[#dc2626] transition-all uppercase shadow-xl">
                    {L.submit}
                </button>
            </div>

            {/* INFRA STATUS (Fixed display) */}
            <div className="mt-8 flex justify-center gap-6 text-gray-300 text-[8px] md:text-xs font-mono font-black italic uppercase">
                ...
                <span>Developer: @Ohsangjo</span>
            </div>
          </div>
        )}
      </main>

      {/* FOOTER TAB BAR (Global Navigation) */}
      ...
      {/* GLOBAL COPYRIGHT */}
      <div className="mt-20 opacity-20 text-black text-[9px] md:text-xl tracking-[1em] pb-32 font-black text-center uppercase">
        Kedheon master | V108.6 Final Empire | @Ohsangjo
      </div>
    </div>
  );
}
