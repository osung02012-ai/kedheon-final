...
// --- [2. 공통 섹션 타이틀 컴포넌트 (에러 방지를 위해 외부 선언)] ---
...
export default function KedheonPortal() {
  ...
  // --- [상태 관리] ---
  const [hasMounted, setHasMounted] = useState(false); // Hydration 에러 방지용
  ...
  // --- [라이프사이클: 데이터 로드 및 에러 방지] ---
  useEffect(() => {
    setHasMounted(true); // 클라이언트 사이드 마운트 완료 확인
  ...
  }, [beomToken, hasMounted]);

  // --- [비즈니스 로직] ---
  if (!hasMounted) return null; // 서버-클라이언트 불일치(Hydration) 에러 원천 차단
  ...
  const purchaseQR = () => {
    ...
    if (confirm(`${QR_PURCHASE_COST} BEOM으로 제국 인증 QR을 확보하시겠습니까?`)) {
  ...
  const registerAsset = () => {
    ...
    alert(isAdRequest ? "공식 광고가 게재되었습니다!" : "자산 등록 완료");
  };
  ...
  return (
    <div className="flex flex-col items-center bg-black min-h-screen text-white p-6 font-sans w-full overflow-x-hidden">
      ...
      <div className="flex gap-4 mb-16 mt-10 justify-center w-full max-w-2xl">
        ...
        <button onClick={() => {setTab('PIONEER'); setViewMode('HUB');}} className={`px-12 py-4 rounded-2xl font-black text-lg transition-all ${tab === 'PIONEER' && viewMode === 'HUB' ? 'bg-[#daa520] text-black shadow-[0_0_30px_rgba(218,165,32,0.6)]' : 'bg-[#111] text-gray-500 hover:text-white'}`}>PIONEER</button>
      </div>
      ...
      <div className="w-full max-w-5xl">
        {tab === 'ROOKIE' ? (
          ...
          <div className="flex flex-col items-center text-center py-20 animate-in fade-in zoom-in duration-500">
            ...
            <h1 className="text-6xl font-black text-[#daa520] tracking-widest mb-6 uppercase">Kedheon Empire</h1>
          ...
          <div className="flex flex-col gap-14 animate-in slide-in-from-bottom-10 duration-700">
            ...
            {viewMode === 'HUB' ? (
              <>
                ...
                <div className="bg-[#111] p-12 rounded-[60px] border border-[#daa520]/40 shadow-2xl flex justify-between items-center relative overflow-hidden group">
                  <div className="relative z-10">
                    ...
                    <div className="mt-10 bg-black/60 px-8 py-4 rounded-2xl border border-white/10 w-fit backdrop-blur-md flex items-center gap-4">
                       ...
                       <span className="text-[#daa520] text-[10px] font-black opacity-50 underline decoration-[#daa520]">1 Pi : {PI_TO_BEOM_RATE}</span>
                    </div>
                  </div>
                ...
                <div className="p-12 bg-[#111] rounded-[60px] border border-white/5 text-center">
                  ...
                  <div className="flex flex-wrap gap-4 justify-center">
                    ...
                    {categories.map((cat) => (
                      <button key={cat} onClick={() => enterTerritory(cat)} className="px-10 py-5 rounded-3xl text-sm font-black bg-[#1a1a1a] text-gray-500 border border-white/5 hover:text-[#daa520] hover:border-[#daa520]/40 transition-all uppercase">ENTER {cat}</button>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-[#111] p-12 rounded-[60px] border border-[#daa520]/20 text-center relative overflow-hidden">
                  <div className="absolute top-6 right-12 text-sm text-gray-600 font-mono font-bold uppercase">Fee: {QR_PURCHASE_COST} BEOM</div>
                  ...
                  <div className="relative w-full max-w-[600px] mx-auto aspect-[1.8/1] rounded-[50px] border border-white/10 overflow-hidden shadow-2xl group">
                    <img src={qrType === 'PERSONAL' ? personalQrImg : businessQrImg} className="absolute inset-0 w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105" alt="QR BG" />
                    <div className={`absolute inset-0 flex flex-col items-center justify-center p-10 transition-all duration-700 ${isQrActive ? 'bg-transparent' : 'bg-black/95 backdrop-blur-3xl'}`}>
                      {isQrActive ? (
                        <div className="bg-white p-5 rounded-[40px] shadow-2xl border-8 border-[#daa520]/30 animate-in zoom-in-50">
                        ...
                        <button onClick={purchaseQR} className="bg-[#daa520] text-black px-16 py-7 rounded-3xl font-black text-xl shadow-xl hover:scale-105 active:scale-95 transition-all">QR 확보하기</button>
                      )}
                    </div>
                  </div>
                ...
                <div className="bg-[#111] p-12 rounded-[60px] border border-white/5">
                  <SectionTitle title="Ecosystem Infrastructure Hub" desc="제국의 8대 핵심 시스템 가동 상태를 확인하고 활동 보상을 채굴하십시오." />
              ...
              <div className="flex flex-col gap-10 animate-in fade-in duration-500">
                ...
                {/* 글쓰기 섹션 */}
                <div className="bg-[#111] p-12 rounded-[60px] border border-white/5">
                  <h4 className="text-[#daa520] font-black text-xl mb-8 flex items-center gap-4 px-2 uppercase tracking-widest">
                    ✍️ New Broadcast to {category}
                  </h4>
                  <div className="flex flex-col gap-6">
                    ...
                    <div className="flex justify-between items-center bg-black/60 p-8 rounded-[40px] border border-[#daa520]/20 backdrop-blur-md">
                      <div className="flex flex-col text-left"><span className="text-sm font-black text-[#daa520] uppercase tracking-widest">Official AD Enforcement</span><span className="text-xs text-gray-500 mt-2 font-bold opacity-70">500 BEOM | TOP FEED PLACEMENT</span></div>
                      <input type="checkbox" checked={isAdRequest} onChange={(e) => setIsAdRequest(e.target.checked)} className="w-10 h-10 accent-[#daa520] cursor-pointer shadow-lg" />
                    </div>
                    <button onClick={registerAsset} className="w-full py-8 rounded-[40px] font-black text-2xl bg-gradient-to-br from-[#daa520] to-[#b8860b] text-black shadow-2xl hover:scale-[1.01] transition-all uppercase tracking-widest">Post to {category} Feed</button>
                  </div>
                </div>
                ...
                <div className="mt-10 space-y-8">
                  {assets.filter(a => category === 'ALL' || a.category === category).length > 0 ? (
                    assets.filter(a => category === 'ALL' || a.category === category).map((a) => (
                      <div key={a.id} className={`p-10 rounded-[50px] border-l-[16px] transition-all duration-500 ${a.isAd ? 'bg-[#daa520]/10 border-[#daa520] shadow-2xl animate-in slide-in-from-right-5' : 'bg-white/5 border-gray-800'}`}>
                        ...
                        <div className="flex justify-between items-center pt-8 border-t border-white/5 text-left">
                    ...
                    <div className="py-24 text-center border border-dashed border-white/10 rounded-[40px] opacity-20 font-black uppercase tracking-[0.5em]">No secured data in this territory</div>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="text-center py-32 opacity-10">
               ...
               <p className="text-xs font-mono tracking-[1.2em] uppercase leading-loose text-white/50">Kedheon Empire | 88-Threads Engine Verified</p>
            </div>
