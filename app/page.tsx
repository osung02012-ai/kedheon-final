...
/** * [KEDHEON EMPIRE V66.0 FINAL LEGACY MASTER]
 * -----------------------------------------------------------
 * 텍스트 규격 지침:
...
export default function KedheonPortal() {
  ...
  useEffect(() => {
    setHasMounted(true);
    const saved = localStorage.getItem('kedheon_v66_final');
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (hasMounted) {
      localStorage.setItem('kedheon_v66_final', JSON.stringify({ token: beomToken, assets }));
    }
  ...
  return (
    <div className="flex flex-col items-center bg-black min-h-screen text-white font-sans w-full pb-40 font-black overflow-x-hidden">
      ...
      <div className="w-full max-w-4xl flex justify-between items-center p-5 sticky top-0 bg-black/95 backdrop-blur-md z-[150] border-b-2 border-[#daa520]">
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <span className="text-white/30 text-[10px] font-mono tracking-widest uppercase">v66.0 Final Legacy</span>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-12 animate-in fade-in text-left">
            <div className="flex flex-col items-center text-center gap-8 py-16 bg-[#111] rounded-[60px] border-4 border-[#daa520] shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 px-8">
                ...
                <p className="text-white text-xl uppercase tracking-widest border-b-2 border-[#daa520] pb-4 inline-block">케데헌 제국 웹3 초대 및 시민권 안내</p>
                <p className="text-white/40 text-sm mt-8 font-sans max-w-lg mx-auto leading-relaxed font-bold italic">파이 네트워크 기반의 독창적인 경제 생태계, 케데헌 제국의 시민이 되기 위한 8단계 절차를 완수하십시오.</p>
              </div>
            </div>
            ...
            <div className="grid gap-6">
              {[
                ...
                { s: "Step 02", t: "Registration Mode", d: "[Continue with phone number]로 보안 가입을 시작하십시오." },
                { s: "Step 03", t: "Region Selection", d: "국가 코드를 [South Korea (+82)]로 정확히 설정하십시오." },
                { s: "Step 04", t: "Account Security", d: "대/소문자 및 숫자를 조합하여 강력한 비밀번호를 생성하십시오." },
                { s: "Step 05", t: "Identity Entry", d: "여권 영문 실명을 입력하고 사용할 고유 ID를 설정하십시오." },
                { s: "Step 06", t: "Invitation Protocol", d: `초대 코드 [ ${PI_INVITE_CODE} ]를 입력하여 제국 입국을 승인받으십시오.`, gold: true },
                { s: "Step 07", t: "Passphrase Vault", d: "지갑 생성 시 주어지는 24개 비밀구절은 오직 종이에만 기록하십시오.", danger: true },
                { s: "Step 08", t: "Mining Activation", d: "번개 버튼을 눌러 채굴을 활성화하고 시민권을 정식 획득하십시오.", gold: true }
              ].map((step, idx) => (
                <div key={idx} className={`p-6 bg-[#111] rounded-[30px] border-2 shadow-xl ${step.gold ? 'border-[#daa520] bg-[#daa520]/5' : step.danger ? 'border-red-600 font-bold' : 'border-white/5'}`}>
          ...
          <div className="flex flex-col gap-14 py-8 animate-in slide-in-from-bottom-5 text-left font-black">
            ...
            <div className="bg-[#111] p-12 rounded-[60px] border-4 border-[#daa520] shadow-2xl flex flex-col md:flex-row justify-between items-center gap-10">
                <div className="text-center md:text-left flex-1 font-black">
                  ...
                  <div className="mt-8 bg-black/60 px-8 py-3 rounded-2xl border border-white/10 inline-block text-xl font-mono italic text-white/50">
                    ≈ {(beomToken / PI_TO_BEOM_RATIO).toFixed(4)} Pi
                  </div>
                </div>
                <div className="flex items-center gap-8">
                  <img src="/kedheon-character.png" className="w-24 h-24 md:w-40 md:h-40 rounded-[40px] border-2 border-white/10" alt="K" />
                  <img src="/beom-token.png" className="w-20 h-20 md:w-32 md:h-32 object-contain animate-pulse" alt="B" />
                </div>
            </div>

            {/* 🌐 01. 범토큰 환전소 (복구 완료) */}
            <SectionHeader num="01" title="EXCHANGE CENTER" desc="파이 네트워크의 기여도를 제국의 범토큰(BEOM)으로 즉시 전환하십시오." />
            <div className="bg-[#111] p-10 rounded-[45px] border-2 border-[#daa520] flex flex-col md:flex-row justify-between items-center gap-8 shadow-2xl">
              <div className="text-left font-black">
                <p className="text-white text-xl uppercase italic mb-2 font-black">Pi to Beom Exchange</p>
                <p className="text-[#daa520] text-sm font-black font-sans uppercase tracking-widest">환율: 1 Pi = 100 BEOM</p>
              </div>
              <button onClick={handleExchange} className="w-full md:w-auto bg-[#daa520] text-black px-16 py-6 rounded-3xl text-xl border-4 border-white active:scale-95 uppercase font-black shadow-xl">Convert 1 Pi</button>
            </div>

            {/* 🌐 02. 보안 인증 (QR 분리 및 설명글) */}
            <SectionHeader num="02" title="SECURE AUTH" desc="시민의 고유 디지털 지문을 활성화하여 보안 익명 결제 시스템을 이용하십시오." />
            <div className="bg-[#111] p-10 rounded-[45px] border-2 border-white/10 flex flex-col items-center gap-10 shadow-xl">
              <div className="flex gap-4 w-full max-w-sm bg-black p-2 rounded-2xl border-2 border-white/5 font-black">
              ...
              {qrType === 'BUSINESS' && (
                <div className="w-full max-w-sm animate-in fade-in">
                  <p className="text-[#daa520] text-sm uppercase mb-3 ml-2 tracking-widest font-black">Enterprise Name</p>
              ...
              <div className={`p-8 bg-black border-4 rounded-[70px] transition-all flex flex-col items-center justify-center w-80 h-80 ${isQrActive ? 'border-[#daa520] shadow-[0_0_60px_rgba(218,165,32,0.4)]' : 'opacity-10 border-white'}`}>
                {isQrActive ? (
                  <>
                    ...
                    <span className="text-[#daa520] text-sm uppercase tracking-widest font-black italic">{qrType === 'BUSINESS' ? bizName : 'Imperial Verified'}</span>
                  </>
                ) : (
                  <p className="text-white/5 text-2xl italic uppercase font-black tracking-[0.5em]">Locked</p>
                )}
              </div>
              <button onClick={() => { if(qrType==='BUSINESS' && !bizName) return alert("기업명을 입력하십시오."); setIsQrActive(true); setBeomToken(p => p - COSTS_BEOM.ACTIVATE_QR); }} className="w-full max-w-sm bg-[#daa520] text-black py-7 rounded-3xl text-xl border-4 border-white active:scale-95 uppercase font-black shadow-2xl">인증 활성화 ({COSTS_BEOM.ACTIVATE_QR} BEOM)</button>
            </div>

            {/* 🌐 03. 창작 게시판 (카테고리 확장 및 Praise 버튼) */}
            <SectionHeader num="03" title="CREATIVE BOARD" desc="영상 및 창작물을 공유하여 제국 경제에 기여하고 파이오니어들의 호응을 받으십시오." />
            <div className="bg-[#111] p-10 rounded-[45px] border-2 border-[#daa520]/30 space-y-6 shadow-2xl">
              <div className="flex gap-2 p-1 bg-black rounded-2xl border border-white/10 font-black font-sans">
              ...
              <div className="flex gap-4">
                ...
                <button className="flex-1 bg-white text-black py-6 rounded-3xl text-sm border-2 border-[#daa520] uppercase font-black font-sans leading-tight shadow-xl">🚩 팬방 개설<br/>({COSTS_BEOM.CREATE_FAN_ROOM})</button>
              </div>
            </div>
              ...
              {assets.map(a => (
                <div key={a.id} className="bg-[#111] p-12 rounded-[60px] border-2 border-white/5 shadow-2xl relative transition-all hover:border-[#daa520]/40">
                  <div className="flex justify-between items-start mb-8 font-black">
                    ...
                    <span className="text-white/20 text-sm font-mono font-bold">{a.timestamp}</span>
                  </div>
                  {a.url && (
              ...
              <div className="space-y-8">
                ...
                {goods[0].reviews.map(r => (
                  <div key={r.id} className="bg-black/50 p-8 rounded-[40px] border-2 border-white/5 shadow-inner transition-all hover:border-[#daa520]/20 font-black">
                    <p className="text-white/80 text-sm italic font-sans mb-4 font-bold leading-relaxed">"{r.text}"</p>
                    <div className="flex justify-between items-center text-[11px] font-mono text-[#daa520] font-black uppercase tracking-[0.2em]">
                      <span>- {r.user}</span>
      ...
      <div className="mt-40 opacity-20 text-sm tracking-[1.5em] uppercase pb-20 font-sans font-black">
        Kedheon Empire | V66.0 Final Legacy | @Ohsangjo
      </div>
    </div>
  );
}
