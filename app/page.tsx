...
export default function KedheonPortal() {
  ...
  const [goods, setGoods] = useState<Good[]>([
    { 
      ...
      reviews: [{ id: 1, user: "Pioneer_K", text: "퀄리티가 압도적입니다!", rating: 5, timestamp: "2026.04.28" }] 
    }
  ]);
  ...
  const postGood = () => {
    ...
    if(beomToken < COSTS_BEOM.SELL_ITEM) return alert("자산이 부족합니다.");
  ...
  const postReview = () => {
    if(!reviewText.trim()) return alert("리뷰를 입력하십시오.");
    if(beomToken < COSTS_BEOM.WRITE_REVIEW) return alert("자산이 부족합니다.");
  ...
  const SectionHeader = ({ num, title, desc }: { num: string; title: string; desc: string }) => (
    <div className="w-full border-t-2 border-white/20 pt-8 mb-6 text-left font-black">
      <h3 className="text-[#daa520] text-xl md:text-3xl uppercase border-l-4 border-[#daa520] pl-3 leading-none italic tracking-tighter mb-2 font-sans font-black">
        {num}. 🌐 {title}
      </h3>
    ...
    <div className="flex flex-col items-center bg-black min-h-screen text-white font-sans w-full pb-40 overflow-x-hidden text-center font-black">
      ...
      <div className="w-full max-w-4xl flex justify-between items-center p-3 md:p-5 sticky top-0 bg-black/95 backdrop-blur-md z-[150] border-b-2 border-[#daa520]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white/30 text-[10px] font-mono tracking-tighter px-2 border-l border-white/10">v61.2</span>
        </div>
      ...
      <div className="w-full max-w-4xl px-4">
        {tab === 'PIONEER' && (
          <div className="flex flex-col gap-8 py-8 animate-in slide-in-from-bottom-5">
            ...
            {/* 01. 환전 */}
            <SectionHeader num="01" title="ACQUIRE BEOM" desc="파이 네트워크의 희소가치를 제국의 자산으로 전환하여, 국경 없는 경제 생태계의 주도권을 확보하십시오." />
            ...
            <div className="bg-[#111] p-5 rounded-[30px] border-2 border-white/10 flex flex-col items-center gap-6 shadow-xl font-black font-sans">
              ...
              {qrType === 'BUSINESS' && (
                <input 
                  ...
                  className="w-full max-w-xs bg-black border-2 border-[#daa520] p-3 rounded-lg text-center text-[#daa520] text-sm outline-none font-black shadow-inner animate-in fade-in" 
                />
              )}
              ...
              <div className={`p-4 bg-black border-2 rounded-[30px] transition-all flex items-center justify-center min-w-[200px] min-h-[200px] ${isQrActive ? 'border-[#daa520] shadow-[0_0_40px_rgba(218,165,32,0.3)]' : 'opacity-10 border-white'}`}>
                ...
                ) : <div className="w-40 h-40 flex items-center justify-center text-white/5 text-xl italic uppercase font-sans tracking-widest">Locked</div>}
              </div>
            ...
            <div className="w-full bg-[#111] p-5 rounded-[30px] border-2 border-[#daa520]/30 space-y-4 mb-4 shadow-lg text-left font-sans font-black">
              ...
              <div className="flex gap-3">
                <button onClick={postCreation} className="flex-[2] py-4 rounded-xl bg-[#daa520] text-black text-sm border-2 border-white uppercase shadow-lg font-black active:scale-95">피드 등록 ({COSTS_BEOM.POST_CREATION} BEOM)</button>
              ...
              {assets.filter(a => category === 'ALL' || a.category === category).map(a => (
                <div key={a.id} className="bg-[#111] rounded-[30px] border-2 p-6 space-y-4 shadow-xl border-white/10 relative text-left hover:border-[#daa520] transition-all font-black">
                  <div className="flex justify-between items-start">
                    <div><span className="text-[8px] bg-white text-black px-2 py-0.5 rounded-full uppercase mr-2 font-sans">{a.category}</span><h4 className="text-2xl text-[#daa520] tracking-tighter mt-1 font-sans">{a.title}</h4></div>
            ...
            {/* 04. 제국 시장 & 후기 */}
            <SectionHeader num="04" title="MERCHANT SQUARE & REVIEWS" desc="제국의 희귀 굿즈를 거래하고 사용자 후기를 확인하십시오. 당신의 물품을 시장에 내놓아 상업 주권을 행사하십시오." />
              ...
              {goods.map(g => (
                <div key={g.id} className="bg-[#111] rounded-[30px] border-2 border-white/10 p-6 shadow-2xl relative flex flex-col gap-4">
                  ...
                  <div className="mt-4 pt-4 border-t border-white/10 space-y-3">
                    <div className="flex justify-between items-center"><p className="text-[#daa520] text-[10px] uppercase font-bold tracking-widest">User Reviews</p><button onClick={() => setShowReviewModal({ open: true, goodId: g.id })} className="text-white/40 text-[9px] underline font-bold">후기 작성</button></div>
                    {g.reviews.map(r => (
                      <div key={r.id} className="bg-black/50 p-3 rounded-xl border border-white/5">
                        ...
                        <p className="text-[#daa520] text-[8px] mt-1 italic font-sans">- {r.user} ({r.timestamp})</p>
                      </div>
                    ))}
                  </div>
            ...
            <button onClick={() => setShowSellModal(true)} className="w-full py-5 rounded-2xl bg-[#daa520] text-black font-black text-lg border-2 border-white shadow-xl active:scale-95 uppercase italic">➕ 제국 시장에 상품 등록 ({COSTS_BEOM.SELL_ITEM} BEOM)</button>
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-black/95 border-2 border-[#daa520] p-1.5 rounded-2xl flex gap-3 z-[200] shadow-[0_0_50px_rgba(218,165,32,0.4)] backdrop-blur-xl animate-in slide-in-from-bottom-10 font-sans font-black">
        {['KEDHEON', 'CIVIL', 'NEXUS', 'VENDOR'].map(app => (
          <button key={app} className={`px-3 py-1.5 rounded-xl text-[9px] transition-all ${app === 'KEDHEON' ? 'bg-[#daa520] text-black' : 'text-white/40'}`}>{app}</button>
        ))}
      </div>
      ...
      {showFanRoomModal && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-4 animate-in zoom-in-95 font-black">
          <div className="bg-[#111] p-8 rounded-[30px] border-4 border-[#daa520] w-full max-w-sm text-center shadow-2xl">
            ...
            <input type="text" value={fanRoomName} onChange={(e) => setFanRoomName(e.target.value)} placeholder="아지트 명칭" className="bg-black border-2 border-white p-4 rounded-xl w-full text-white text-center mb-6 outline-none font-black" />
            <div className="flex gap-2 font-sans font-black"><button onClick={() => setShowFanRoomModal(false)} className="flex-1 py-3 rounded-lg text-xs bg-white/10 border border-white/20 uppercase">Cancel</button><button onClick={() => { if(!fanRoomName.trim()) return alert("명칭 필수"); setBeomToken(p => p - COSTS_BEOM.CREATE_FAN_ROOM); setShowFanRoomModal(false); setFanRoomName(''); alert("팬방 개설 완료!"); }} className="flex-1 py-3 rounded-lg text-xs bg-[#daa520] text-black border-2 border-white uppercase shadow-lg">Create ({COSTS_BEOM.CREATE_FAN_ROOM})</button></div>
          </div>
        </div>
      )}
      ...
      {showReviewModal.open && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-4 animate-in zoom-in-95 font-black">
          <div className="bg-[#111] p-8 rounded-[30px] border-4 border-white w-full max-w-sm text-center">
            ...
            <div className="flex gap-2 font-sans font-black"><button onClick={() => setShowReviewModal({ open: false, goodId: null })} className="flex-1 py-3 rounded-lg text-xs bg-white/10 border border-white/20 uppercase">Cancel</button><button onClick={postReview} className="flex-1 py-3 rounded-lg text-xs bg-[#daa520] text-black border-2 border-white uppercase shadow-lg">Post</button></div>
          </div>
        </div>
      )}
      ...
      {showSellModal && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-4 animate-in zoom-in-95 font-black">
          <div className="bg-[#111] p-8 rounded-[30px] border-4 border-[#daa520] w-full max-w-sm text-center">
            <h3 className="text-[#daa520] text-xl mb-6 italic uppercase font-sans">Market Registration</h3>
            ...
            <div className="flex gap-2 mt-6 font-sans font-black"><button onClick={() => setShowSellModal(false)} className="flex-1 py-3 rounded-lg text-xs bg-white/10 border border-white/20 uppercase">Cancel</button><button onClick={postGood} className="flex-1 py-3 rounded-lg text-xs bg-[#daa520] text-black border-2 border-white uppercase shadow-lg">Post Sale</button></div>
          </div>
        </div>
      )}

      <div className="mt-24 opacity-30 text-center w-full pb-10 font-mono text-white text-[8px] tracking-[0.8em] uppercase font-black font-sans">KEDHEON EMPIRE | V61.2 ABSOLUTE CITADEL | ohsangjo</div>
    </div>
  );
}
