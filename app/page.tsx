...
export default function KedheonPortal() {
  ...
  const project: Project = { id: 1, name: "PI-VENDORS-HUB", target: 1000000, current: 928000, status: "OPEN" };
  ...
  const t = lang === 'KO' ? {
    ...
    postBtn: "방송하기", supportBtn: "👑 황금 찬양", enterBtn: "제국 입국하기"
  } : {
  ...
  useEffect(() => {
    setHasMounted(true);
    const saved = localStorage.getItem('k_v17_final');
    if (saved) {
  ...
  useEffect(() => {
    if (hasMounted) localStorage.setItem('k_v17_final', JSON.stringify({ token: beomToken, assets, contributed: contributedBeom }));
  ...
  const SectionHeader = ({ num, title, desc, icon }: any) => (
    <div className="w-full flex flex-col items-start gap-4 md:gap-6 mb-12 md:mb-20 pt-20 md:pt-32 border-t border-white/5">
      ...
      <p className="text-gray-500 font-bold text-sm md:text-2xl opacity-80 pl-2 md:pl-4 max-w-5xl leading-relaxed">{desc}</p>
    </div>
  );
  ...
  return (
    <div className="flex flex-col items-center bg-black min-h-screen text-white font-sans w-full overflow-x-hidden pb-40">
      ...
      {/* 1. 와이드 글로벌 헤더 */}
      <div className="w-full max-w-6xl flex justify-between items-center p-6 md:p-12 sticky top-0 bg-black/90 backdrop-blur-2xl z-[150] border-b border-white/5">
        <button onClick={() => setLang(l => l === 'KO' ? 'EN' : 'KO')} className="text-[#daa520] font-black text-xs md:text-base border-2 border-[#daa520]/40 px-6 py-2 md:px-10 md:py-3 rounded-full hover:bg-[#daa520] hover:text-black transition-all">
          {lang === 'KO' ? "ENGLISH" : "한국어"}
        </button>
        <div className="flex gap-4 md:gap-8">
          <button onClick={() => setTab('ROOKIE')} className={`px-6 py-2 md:px-12 md:py-3 rounded-xl font-black text-xs md:text-xl transition-all ${tab === 'ROOKIE' ? 'bg-[#daa520] text-black shadow-xl' : 'bg-[#111] text-gray-500'}`}>ROOKIE</button>
          <button onClick={() => setTab('PIONEER')} className={`px-6 py-2 md:px-12 md:py-3 rounded-xl font-black text-xs md:text-xl transition-all ${tab === 'PIONEER' ? 'bg-[#daa520] text-black shadow-xl' : 'bg-[#111] text-gray-500'}`}>PIONEER</button>
        </div>
      </div>
      ...
      <div className="w-full max-w-6xl flex flex-col gap-24 md:gap-40 px-6 md:px-10 mt-12 md:mt-24">
        {tab === 'ROOKIE' ? (
          <div className="flex flex-col items-center text-center py-40">
            ...
            <h1 className="text-5xl md:text-9xl font-black text-[#daa520] tracking-widest italic mb-12 leading-none uppercase">Kedheon Empire</h1>
            <button onClick={() => setTab('PIONEER')} className="bg-[#daa520] text-black px-12 py-6 md:px-40 md:py-16 rounded-full font-black text-2xl md:text-6xl shadow-2xl hover:scale-105 transition-all">{t.enterBtn}</button>
          </div>
        ) : (
          <div className="flex flex-col gap-32 md:gap-60 animate-in slide-in-from-bottom-20 duration-1000">
            ...
            <div className="bg-[#111] p-10 md:p-24 rounded-[50px] md:rounded-[100px] border border-[#daa520]/40 shadow-2xl flex flex-col lg:flex-row justify-between items-center gap-12 relative overflow-hidden">
               <div className="flex flex-col items-center lg:items-start gap-6 md:gap-12 z-10 text-center lg:text-left">
                  ...
                  <p className="text-[#daa520] font-black text-5xl md:text-[10rem] lg:text-[11rem] tracking-tighter leading-none">{beomToken.toLocaleString()} BEOM</p>
                  <div className="bg-black/60 px-8 py-4 md:px-16 md:py-8 rounded-2xl md:rounded-[45px] border border-white/5 shadow-2xl text-xl md:text-5xl font-mono text-gray-400 italic">≈ {(beomToken / 314.1592).toFixed(4)} Pi</div>
               </div>
            ...
            <div className="flex flex-col w-full">
              ...
              <div className="w-full bg-[#111] p-10 md:p-24 rounded-[50px] md:rounded-[100px] flex flex-col items-center gap-12 md:gap-24 shadow-inner border border-white/5">
                <div className="flex gap-4 md:gap-10 bg-black p-3 md:p-6 rounded-2xl md:rounded-[60px] w-full max-w-4xl shadow-3xl font-black">
                ...
                <div className={`p-10 md:p-20 rounded-[40px] md:rounded-[90px] bg-black border-2 md:border-4 transition-all ${isQrActive ? 'border-[#daa520]' : 'opacity-20'}`}>
            ...
            <div className="flex flex-col w-full">
              ...
              <div className="w-full bg-gradient-to-br from-[#111] to-black p-10 md:p-24 rounded-[50px] md:rounded-[100px] border border-[#daa520]/20 shadow-3xl space-y-12 md:space-y-24 text-left">
            ...
            <div className="flex flex-col w-full">
              ...
              <div className="w-full bg-[#111] p-10 md:p-24 rounded-[50px] md:rounded-[120px] border border-[#daa520]/20 flex flex-col items-center gap-10 md:gap-20 shadow-3xl relative overflow-hidden group">
            ...
            <div className="flex flex-col w-full">
              ...
              <div className="grid grid-cols-3 md:grid-cols-6 gap-4 md:gap-8 w-full mb-16 md:mb-32">
                {cats.map(cat => (
                  <button key={cat} onClick={() => setCategory(cat)} className={`py-6 md:py-14 rounded-2xl md:rounded-[55px] font-black text-[10px] md:text-2xl tracking-widest border-2 transition-all ${category === cat ? 'bg-[#daa520] text-black border-[#daa520] shadow-3xl scale-110' : 'bg-[#111] text-gray-500 border-white/5 hover:border-[#daa520]/50'}`}>{cat}</button>
                ))}
              </div>
              ...
              <div className="w-full bg-[#111] p-10 md:p-24 rounded-[40px] md:rounded-[120px] border border-white/10 space-y-10 md:space-y-16 text-left mb-20 md:mb-40 shadow-3xl">
                <input type="text" value={newT} onChange={(e) => setNewT(e.target.value)} placeholder="TITLE" className="bg-black p-6 md:p-14 rounded-2xl md:rounded-[65px] border border-white/5 w-full text-xl md:text-5xl outline-none focus:border-[#daa520] font-black text-white" />
                <textarea value={newD} onChange={(e) => setNewD(e.target.value)} placeholder="DETAILS" className="bg-black p-6 md:p-14 rounded-2xl md:rounded-[65px] border border-white/5 w-full text-lg md:text-4xl h-40 md:h-80 outline-none focus:border-[#daa520] resize-none text-gray-300 font-bold" />
                <button onClick={() => { if(!newT.trim()) return alert("제목!"); setAssets([{id: Date.now(), title: newT, desc: newD, category, beomSupport: 0, timestamp: new Date().toLocaleDateString()}, ...assets]); setBeomToken(p=>p-10); setNewT(''); setNewD(''); }} className="w-full py-8 md:py-16 rounded-[30px] md:rounded-[80px] font-black text-2xl md:text-6xl bg-gradient-to-r from-[#daa520] to-[#b8860b] text-black shadow-3xl uppercase transition-all">{t.postBtn}</button>
              </div>

              <div className="w-full space-y-24 md:space-y-48">
                <button onClick={() => setCategory('ALL')} className={`w-full py-8 md:py-16 rounded-3xl md:rounded-[70px] font-black text-2xl md:text-5xl border-4 transition-all mb-12 md:mb-24 ${category === 'ALL' ? 'border-[#daa520] text-[#daa520] bg-[#daa520]/5' : 'border-white/5 text-gray-600'}`}>ENTER ALL FEED</button>
                {assets.filter(a => category === 'ALL' || a.category === category).map(a => (
                  <div key={a.id} className="bg-[#111] rounded-[50px] md:rounded-[150px] border border-white/5 shadow-2xl p-10 md:p-28 space-y-10 md:space-y-20 text-left relative animate-in slide-in-from-bottom-20 duration-1000">
                    ...
                    <div className="pt-10 md:pt-24 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
                      <button onClick={() => { if(beomToken<100) return alert("!"); setAssets(assets.map(i=>i.id===a.id?{...i, beomSupport:i.beomSupport+100}:i)); setBeomToken(p=>p-100); }} className="bg-gradient-to-r from-[#daa520] to-[#b8860b] text-black px-12 py-5 md:px-24 md:py-12 rounded-2xl md:rounded-[50px] font-black text-xl md:text-4xl hover:scale-110 shadow-3xl">{t.supportBtn}</button>
      ...
      <div className="mt-80 opacity-20 text-center w-full pb-60 font-mono tracking-[1em] md:tracking-[3em] uppercase text-white/50 text-xs md:text-xl">Kedheon Empire | Final v17.0 Full Uncut Master</div>
    </div>
  );
}
