...
/**
 * [경제 시스템 변수]
 * 파이의 실질 가치 실현 속도에 따라 이 숫자만 수정하십시오.
 * 100 : 표준(100:1) | 314 : 상징성 | 1000 : 초고가치 대응
 */
        ...
        {tab === 'ROOKIE' ? (
          <div className="flex flex-col items-center py-10">
            ...
            {!showOnboarding ? (
              ...
              <div className="w-full bg-[#111] p-6 rounded-[30px] border-2 border-white/20 space-y-6 mb-12 animate-in slide-in-from-bottom-5">
            ...
            <div className="w-full opacity-70 scale-95 pointer-events-none md:pointer-events-auto text-left">
          ...
          <div className="flex flex-col gap-8 py-8 animate-in slide-in-from-bottom-5">
            ...
            <div className="bg-[#111] p-6 rounded-[30px] border-2 border-[#daa520] shadow-xl flex flex-col md:flex-row justify-between items-center gap-6">
               <div className="text-center md:text-left flex-1 font-black">
                  ...
                  <p className="text-[#daa520] text-4xl md:text-7xl tracking-tighter leading-none font-black font-sans">
                    {beomToken.toLocaleString()} <span className="text-xl md:text-3xl">BEOM</span>
                  </p>
                  <div className="mt-2 bg-black px-4 py-1 rounded-lg border border-white/20 inline-block text-xs font-mono text-white/60 italic">
                    ≈ {(beomToken / PI_TO_BEOM_RATIO).toFixed(4)} Pi (1:{PI_TO_BEOM_RATIO})
                  </div>
               </div>
               <div className="flex items-center gap-4"><img src="/kedheon-character.png" className="w-16 h-16 md:w-24 md:h-24 rounded-xl border border-white/20 shadow-lg" alt="Char" /><img src="/beom-token.png" className="w-14 h-14 md:w-24 md:h-24" alt="Token" /></div>
            </div>

            {/* 제국 선언문 (가독성 최적화) */}
            <div className="w-full bg-[#111] p-6 md:p-10 rounded-[40px] border-2 border-[#daa520]/50 shadow-lg">
              <h2 className="text-[#daa520] text-xl md:text-4xl uppercase italic mb-5 tracking-tighter font-black font-sans">누리고 즐기고 선점하세요</h2>
              <div className="space-y-3 text-white/80 text-sm md:text-xl font-bold leading-relaxed tracking-tight font-black">
            ...
            <div className="flex flex-col w-full">
              <SectionHeader num="01" title="ACQUIRE BEOM" desc="BEOM 토큰을 확보하여 주권을 행사하십시오." />
              <div className="bg-[#111] p-6 rounded-[30px] border-2 border-white/10 shadow-lg grid md:grid-cols-2 gap-6 items-center">
                <div className="text-left"><p className="text-white text-base md:text-2xl uppercase italic leading-tight font-black font-sans">파이를 범으로 환전</p>
                <p className="text-[#daa520] text-[10px] md:text-sm italic font-bold">1 Pi = {PI_TO_BEOM_RATIO} BEOM</p></div>
            ...
            <div className="flex flex-col w-full">
              ...
              <div className="bg-[#111] p-5 rounded-[30px] border-2 border-white/10 flex flex-col items-center gap-6 shadow-xl">
                ...
                {qrType === 'BUSINESS' && <input type="text" value={bizName} onChange={(e) => setBizName(e.target.value)} placeholder="ENTER BUSINESS NAME" className="w-full max-w-xs bg-black border-2 border-[#daa520] p-3 rounded-lg text-center text-[#daa520] text-sm outline-none font-black" />}
                ...
                <button onClick={() => { setIsQrActive(true); setBeomToken(p => p - 50); }} className="bg-[#daa520] text-black px-8 py-3 rounded-xl text-sm md:text-lg border-2 border-white active:scale-95 uppercase font-black font-sans">인증 활성화 (50 BEOM)</button>
              </div>
            </div>

            {/* 03. 창작 및 방송 허브 */}
            <div className="flex flex-col w-full text-left">
              <SectionHeader num="03" title="CREATIVE HUB" desc="창작물을 제국 전역에 송출하고 찬양을 확보하십시오." />
              <div className="w-full bg-[#111] p-5 rounded-[30px] border-2 border-[#daa520]/30 space-y-4 mb-8 shadow-lg">
                ...
                <button onClick={postContent} className="w-full py-3.5 rounded-xl bg-[#daa520] text-black text-sm border-2 border-white shadow-lg active:scale-95 uppercase font-black font-sans">피드 등록 (10 BEOM)</button>
              </div>
              <div className="grid grid-cols-4 md:grid-cols-6 gap-1.5 mb-6 font-sans">{cats.map(cat => ( <button key={cat} onClick={() => setCategory(cat)} className={`py-2.5 rounded-lg text-[8px] md:text-[10px] border transition-all font-black ${category === cat ? 'bg-[#daa520] text-black border-[#daa520]' : 'bg-black text-white/20 border-white/10'}`}>{cat}</button> ))}</div>
              <FeedList showSupport={true} />
            </div>

            {/* 04. 팬덤 굿즈 */}
            <div className="flex flex-col w-full text-left">
              <SectionHeader num="04" title="FANDOM GOODS" desc="제국의 상징물을 소유하고 가치를 공유하십시오." />
              <div className="grid grid-cols-2 gap-4 mb-8">
                {goods.map(g => (
                  <div key={g.id} className="bg-[#111] rounded-[25px] border border-white/20 p-4 shadow-lg flex flex-col items-center gap-3 relative font-black">
                    <div className="absolute top-3 right-3 bg-[#daa520] text-black px-2 py-0.5 rounded-full text-[6px] font-black italic border border-white">SAMPLE</div>
                    <img src={g.img} className="w-full h-24 md:h-32 object-contain bg-black rounded-xl border border-white/5" alt="G" />
                    <div className="w-full text-left">
                      <p className="text-[#daa520] text-[6px] uppercase opacity-40 font-black tracking-widest font-sans">Seller: {g.seller}</p>
                      <h4 className="text-white font-black text-xs md:text-base uppercase tracking-tighter truncate font-sans">{g.name}</h4>
                      <p className="text-[#daa520] text-sm md:text-xl font-black font-sans">{g.price.toLocaleString()} <span className="text-[8px]">BEOM</span></p>
                    </div>
              ...
              <button onClick={() => setShowCreateModal(true)} className="w-full py-4 rounded-xl bg-[#daa520] text-black font-black text-base border-2 border-white shadow-lg active:scale-95 uppercase italic font-black font-sans">➕ 신규 영토 개설 (500 BEOM)</button>
            </div>
          </div>
        )}
      </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-4 animate-in zoom-in-95">
          <div className="bg-[#111] p-8 rounded-[30px] border-4 border-[#daa520] w-full max-w-sm text-center font-black shadow-2xl">
            ...
            <div className="flex gap-2 font-sans"><button onClick={() => setShowCreateModal(false)} className="flex-1 py-3 rounded-lg text-xs bg-white/5 border border-white/20 uppercase font-black">Cancel</button><button onClick={() => { setShowCreateModal(false); setCreateTitle(''); alert("개설 완료"); }} className="flex-1 py-3 rounded-lg text-xs bg-[#daa520] text-black border-2 border-white font-black uppercase shadow-lg">Create</button></div>
          </div>
        </div>
      )}

      {/* 최종 푸터 */}
      <div className="mt-16 opacity-30 text-center w-full pb-8 font-mono text-white text-[8px] tracking-[0.8em] uppercase font-black">KEDHEON EMPIRE | V60.3 MASTER | ohsangjo</div>
    </div>
  );
}
