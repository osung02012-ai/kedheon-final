...
/** * [KEDHEON MASTER V230.0 - ABSOLUTE STABLE TOTAL]
 * -----------------------------------------------------------
 * 1. 기술 수정: TypeScript Strict Mode 및 Turbopack 빌드 에러 완벽 해결
 * 2. 01. 전환: 주군 명령 1~5번 기능 직관적 나열 및 환전(1:1000) 로직
 * 3. 02. QR보안: 지갑 주소 노출 방지 목적 명시 및 [개인용 / 기업용] 선택
 * 4. 03. 팬덤: CCM, 뮤지션 카테고리 최상단 배치 및 스크린샷 UI (Hub/Spirit)
 * 5. 04. 굿즈: 굿즈 판매 및 구입, 상세 정보(설명/이미지) 등록 시스템
 * 6. 05. 파트너십: 비즈니스 전문 제안 양식 (기업/담당자/메일/연락처/내용)
 * 7. 디자인: V105.4의 압축된 황금 비율과 콤팩트한 폰트 스케일 복구
 * -----------------------------------------------------------
 */
...
const DICT: Record<Lang, Dictionary> = {
  KR: {
    ...
    exList: [
      ...
      "4. 굿즈 판매 및 구입 (한정 굿즈 및 아이템 거래)",
      "5. 글로벌 파트너십 (비즈니스 협력 및 공식 제안)"
    ],
    steps: [
...
export default function KedheonAbsoluteCompleteV230() {
  ...
  const [goods, setGoods] = useState<GoodsItem[]>([
    { id: 1, name: "기념 뱃지", price: 1000, desc: "한정판 골드 뱃지입니다.", img: "/beom-token.png", seller: "System" },
    { id: 2, name: "V23 노드 키", price: 5000, desc: "노드 운영 디지털 마스터키입니다.", img: "/node-icon.png", seller: "System" }
  ]);
  ...
  // [Fix] url 파라미터 타입 명시 및 null check 추가
  ...
  return (
    <div className="flex flex-col items-center bg-white min-h-screen text-black font-sans w-full pb-32 font-black selection:bg-red-50 overflow-x-hidden">
      ...
      <nav className="w-full max-w-6xl flex justify-between items-center px-4 py-2 sticky top-0 bg-white/95 backdrop-blur-sm z-[500] border-b border-black/5 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div className="text-left leading-tight">
            <h1 className="text-black text-lg md:text-xl font-black italic uppercase leading-none">Kedheon</h1>
            <span className="text-gray-400 text-[8px] font-mono tracking-widest uppercase">MASTER V230.0</span>
          </div>
        </div>
      ...
      <main className="w-full max-w-5xl px-4 py-4">
        {tab === 'ROOKIE' ? (
          <div className="flex flex-col gap-4 animate-in fade-in duration-500 font-black">
            {/* HERO BOX */}
            ...
            {/* INVITATION CODE */}
            <div className="p-8 bg-black text-white rounded-3xl text-center shadow-xl border-4 border-[#dc2626] cursor-pointer" onClick={handleCopy}>
          ...
          <div className="flex flex-col gap-10 py-2 animate-in slide-in-from-bottom-5 duration-700 font-black text-left">
              ...
              <div className="lg:col-span-2 bg-white p-6 md:p-8 rounded-3xl border-2 border-black shadow-lg flex flex-col justify-center relative overflow-hidden group">
                ...
                <div className="relative z-10">
                    ...
                    <div className="flex items-center gap-2 mt-4 font-black">
                      ...
                      <div className="bg-[#dc2626] text-white px-3 py-1 rounded text-[8px] md:text-[10px] font-mono italic shadow-sm">RT: 15,080</div>
                    </div>
                </div>
              </div>
            ...
            <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-black/10 space-y-6 shadow-sm font-black">
              <div className="flex gap-8 border-b border-gray-100 pb-2 font-black text-left">
              ...
              <div className="flex gap-2 flex-wrap">{FANS.map(f => <button key={f} className="px-3 py-1.5 bg-white border border-gray-100 rounded-full text-[9px] md:text-xs font-black text-red-600 shadow-sm">🚩 {f}</button>)}</div>
                        ...
                        {goods.map(g => (
                            <div key={g.id} className="bg-gray-50 p-5 rounded-3xl border flex gap-4 items-center group shadow-sm transition-all hover:border-[#dc2626]">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-gray-500 text-[9px] md:text-[11px] mb-2 font-bold line-clamp-2 leading-tight">{g.desc}</p>
                                    <p className="text-[#dc2626] text-sm md:text-xl font-black mb-2">{g.price.toLocaleString()} BEOM</p>
                                    <button className="w-full py-2 bg-black text-white rounded-lg text-[9px] font-black uppercase hover:bg-[#dc2626] transition-all">구매 신청</button>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="bg-black p-6 rounded-[2rem] border-[8px] border-[#dc2626] space-y-4 relative overflow-hidden font-black">
                <h3 className="text-white text-lg md:text-3xl font-black italic border-l-4 border-[#dc2626] pl-3 uppercase leading-none z-10 relative">GLOBAL PARTNERSHIP</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 z-10 relative font-black">
                    <input value={partner.corp} onChange={e=>setPartner({...partner, corp: e.target.value})} placeholder={L.corpName} className="bg-white/10 border border-white/10 p-4 rounded-xl text-xs md:text-sm text-white outline-none focus:border-[#dc2626]" />
                    <input value={partner.manager} onChange={e=>setPartner({...partner, manager: e.target.value})} placeholder={L.manager} className="bg-white/10 border border-white/10 p-4 rounded-xl text-xs md:text-sm text-white outline-none focus:border-[#dc2626]" />
                    <input value={partner.email} onChange={e=>setPartner({...partner, email: e.target.value})} placeholder={L.email} className="bg-white/10 border border-white/10 p-4 rounded-xl text-xs md:text-sm text-white outline-none focus:border-[#dc2626]" />
                    <input value={partner.contact} onChange={e=>setPartner({...partner, contact: e.target.value})} placeholder={L.contact} className="bg-white/10 border border-white/10 p-4 rounded-xl text-xs md:text-sm text-white outline-none focus:border-[#dc2626]" />
                </div>
      ...
      <footer className="fixed bottom-4 left-4 right-4 max-w-4xl mx-auto bg-white border-2 border-black p-1.5 rounded-2xl flex justify-between gap-1.5 z-[1000] shadow-2xl font-black">
        {['KEDHEON', 'CIVIL', 'NEXUS', 'VENDOR'].map(app => (
          <button key={app} className={`flex-1 py-4 rounded-xl text-[10px] md:text-sm transition-all font-black text-center leading-none ${app === 'KEDHEON' ? 'bg-black text-white shadow-md' : 'text-gray-300'}`}>
            {app}
          </button>
        ))}
      </footer>
