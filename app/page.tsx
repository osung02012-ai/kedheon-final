 ...
 * 파이의 시장 가치가 실현됨에 따라 이 수치만 수정하면 제국 전체 물가가 자동 조절됩니다.
 */
const PI_TO_BEOM_RATIO = 100; 

/** [서비스 원가 가이드라인 (Pi 기준)] */
const COSTS_PI = {
  POST_CREATION: 0.1,     // 팬심/창작 피드 등록 원가 (10 BEOM)
  SUPPORT: 1.0,           // 찬양/후원 원가 (100 BEOM)
  CREATE_FAN_ROOM: 5.0,   // 팬방(아지트) 개설 원가 (500 BEOM)
  SELL_ITEM: 0.2,         // 마켓 상품 등록 원가 (20 BEOM)
  WRITE_REVIEW: 0.05      // 리뷰 작성 원가 (5 BEOM)
};
...
export default function KedheonPortal() {
  ...
  const [goods, setGoods] = useState<Good[]>([
    { 
      ...
      reviews: [{ id: 1, user: "Pioneer_K", text: "실물이 훨씬 웅장합니다. 가치가 느껴지네요.", rating: 5, timestamp: "2026.04.28" }] 
    }
  ]);
  ...
  const postCreation = () => {
    ...
    alert("제국 통신망에 당신의 영감이 성공적으로 기록되었습니다.");
  };

  const createFanRoom = () => {
    if(!fanRoomName.trim()) return alert("팬방의 고유 명칭을 입력하십시오.");
    if(beomToken < COSTS_BEOM.CREATE_FAN_ROOM) return alert(`개설 자산 부족 (필요: ${COSTS_BEOM.CREATE_FAN_ROOM} BEOM)`);
    setBeomToken(p => p - COSTS_BEOM.CREATE_FAN_ROOM);
    alert(`[${fanRoomName}] 팬방이 성공적으로 개설되었습니다. 동료들을 결집하십시오!`);
    setShowFanRoomModal(false); setFanRoomName('');
  };

  const postGood = () => {
    if(!sellName.trim() || sellPrice <= 0) return alert("상품명과 정당한 가격을 입력하십시오.");
    ...
    alert("당신의 상품이 제국 시장(Merchant Square)에 상장되었습니다.");
  };

  const postReview = () => {
    if(!reviewText.trim()) return alert("후기 내용을 작성하십시오.");
    if(beomToken < COSTS_BEOM.WRITE_REVIEW) return alert(`리뷰 등록 자산 부족 (필요: ${COSTS_BEOM.WRITE_REVIEW} BEOM)`);
    ...
    alert("구매 후기가 시장 데이터베이스에 기록되었습니다.");
  };
  ...
  const activateQr = () => {
    const cost = 50; 
    if (beomToken < cost) return alert("보안 인증 활성화를 위한 자산이 부족합니다.");
    setBeomToken(p => p - cost);
    setIsQrActive(true);
  };
    ...
    <div className="flex flex-col items-center bg-black min-h-screen text-white font-sans w-full pb-40 overflow-x-hidden text-center font-black">
      ...
      <div className="w-full max-w-4xl flex justify-between items-center p-3 md:p-5 sticky top-0 bg-black/95 backdrop-blur-md z-[150] border-b-2 border-[#daa520]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white/30 text-[10px] font-mono tracking-tighter px-2 border-l border-white/10 italic">v61.6</span>
        </div>
      ...
      <div className="w-full max-w-4xl px-4">
        {tab === 'ROOKIE' ? (
          <div className="flex flex-col items-center py-10 animate-in fade-in duration-500">
            ...
            {!showOnboarding ? <button onClick={() => setShowOnboarding(true)} className="bg-white text-black px-8 py-3 rounded-full text-base border-2 border-[#daa520] active:scale-95 mb-12 uppercase font-black font-sans">시민권 획득 가이드</button>
            : (
              <div className="w-full bg-[#111] p-6 rounded-[30px] border-2 border-white/20 space-y-6 mb-12 font-sans text-left">
                <h2 className="text-[#daa520] text-xl md:text-3xl uppercase italic font-black text-center font-sans">Onboarding</h2>
                <div className="space-y-3 text-sm md:text-base font-bold text-white/80 font-sans">
            ...
            <div className="w-full opacity-30 italic py-10 font-sans">시민권 획득 후 전체 기능이 활성화됩니다.</div>
          </div>
        ) : (
          <div className="flex flex-col gap-8 py-8 animate-in slide-in-from-bottom-5">
            ...
            <div className="bg-[#111] p-6 rounded-[30px] border-2 border-[#daa520] shadow-xl flex flex-col md:flex-row justify-between items-center gap-6">
               <div className="text-center md:text-left flex-1 font-black">
                  ...
                  <div className="mt-2 bg-black px-4 py-1 rounded-lg border border-white/20 inline-block text-[10px] font-mono text-white/60 italic font-sans">≈ {(beomToken / PI_TO_BEOM_RATIO).toFixed(4)} Pi</div>
               </div>
               <img src="/kedheon-character.png" className="w-16 h-16 md:w-24 md:h-24 rounded-xl border border-white/20 shadow-lg" alt="Char" />
            </div>

            {/* 01. 환전 */}
            <SectionHeader num="01" title="ACQUIRE BEOM" desc="파이 네트워크의 가치를 제국의 범(BEOM) 자산으로 전환하여, 경제적 주권을 확립하십시오." />
            <div className="bg-[#111] p-6 rounded-[30px] border-2 border-white/10 shadow-lg grid md:grid-cols-2 gap-6 items-center">
              <div className="text-left font-black font-sans"><p className="text-white text-base md:text-2xl uppercase italic font-sans">PI TO BEOM</p><p className="text-[#daa520] text-[10px] italic font-sans">1 Pi = {PI_TO_BEOM_RATIO} BEOM</p></div>
            ...
            {/* 02. 보안 인증 (기업명 입력 박스 포함) */}
            <SectionHeader num="02" title="SECURE AUTH" desc="고유한 디지털 지문을 통해 신분을 증명하고 보안 익명 결제를 활성화하십시오." />
            <div className="bg-[#111] p-5 rounded-[30px] border-2 border-white/10 flex flex-col items-center gap-6 shadow-xl font-black font-sans">
              ...
              {qrType === 'BUSINESS' && (
                <input type="text" value={bizName} onChange={(e) => setBizName(e.target.value)} placeholder="ENTER BUSINESS NAME" className="w-full max-w-xs bg-black border-2 border-[#daa520] p-3 rounded-lg text-center text-[#daa520] text-sm outline-none shadow-inner font-sans font-black animate-in fade-in" />
              )}
              ...
              <div className={`p-4 bg-black border-2 rounded-[30px] transition-all flex items-center justify-center min-w-[200px] min-h-[200px] ${isQrActive ? 'border-[#daa520] shadow-[0_0_40px_rgba(218,165,32,0.3)]' : 'opacity-10 border-white'}`}>
                ...
                : <div className="w-40 h-40 flex items-center justify-center text-white/5 text-xl italic uppercase tracking-widest font-sans">Locked</div>}
              </div>
              <button onClick={activateQr} className="bg-[#daa520] text-black px-8 py-3 rounded-xl text-sm md:text-lg border-2 border-white active:scale-95 uppercase font-black font-sans">인증 활성화 (50 BEOM)</button>
            </div>

            {/* 03. 팬심 & 창작 아지트 */}
            <SectionHeader num="03" title="FAN & CREATIVE HIDEOUT" desc="영상과 창작물을 공유하여 팬심을 증명하고, 독자적인 팬방(아지트)을 개설하여 시민들의 찬양을 이끌어내십시오." />
            <div className="w-full bg-[#111] p-5 rounded-[30px] border-2 border-[#daa520]/30 space-y-4 mb-4 shadow-lg text-left font-sans font-black">
              ...
              <input type="text" value={newUrl} onChange={(e) => setNewUrl(e.target.value)} placeholder="영상 링크 혹은 이미지 URL (선택사항)" className="bg-black border border-white/20 p-3 rounded-lg w-full text-xs text-white/40 outline-none" />
              ...
              <div className="grid grid-cols-4 md:grid-cols-6 gap-1.5 py-2 font-bold uppercase font-sans">
                {cats.map(cat => ( <button key={cat} onClick={() => setCategory(cat)} className={`py-2 rounded-lg text-[8px] border transition-all font-sans ${category === cat ? 'bg-[#daa520] text-black border-[#daa520]' : 'bg-black text-white/20 border-white/10'}`}>{cat}</button> ))}
              </div>
              <div className="flex gap-3">
                <button onClick={postCreation} className="flex-[2] py-4 rounded-xl bg-[#daa520] text-black text-sm border-2 border-white uppercase shadow-lg font-black active:scale-95 font-sans">피드 등록 ({COSTS_BEOM.POST_CREATION} BEOM)</button>
                <button onClick={() => setShowFanRoomModal(true)} className="flex-1 py-4 rounded-xl bg-white text-black text-[10px] border-2 border-[#daa520] uppercase active:scale-95 font-black font-sans">🚩 팬방 개설</button>
              </div>
            </div>
              ...
              {assets.filter(a => category === 'ALL' || a.category === category).map(a => (
                <div key={a.id} className="bg-[#111] rounded-[30px] border-2 p-6 space-y-4 shadow-xl border-white/10 relative text-left hover:border-[#daa520] transition-all font-black font-sans">
                  <div className="flex justify-between items-start font-sans">
                    <div><span className="text-[8px] bg-white text-black px-2 py-0.5 rounded-full uppercase mr-2 font-black font-sans">{a.category}</span><h4 className="text-2xl text-[#daa520] tracking-tighter uppercase mt-1 font-sans">{a.title}</h4></div>
                  ...
                  <div className="pt-4 border-t-2 border-white/5 flex justify-between items-center font-sans">
              ...
              {goods.map(g => (
                <div key={g.id} className="bg-[#111] rounded-[30px] border-2 border-white/10 p-6 shadow-2xl flex flex-col gap-4 relative">
                  ...
                  <div>
                    ...
                    <p className="text-white/40 text-[10px] mt-1 italic break-words">{g.desc}</p>
                  </div>
                  <div className="mt-2 pt-4 border-t border-white/10 space-y-3">
                    <div className="flex justify-between items-center font-sans"><p className="text-[#daa520] text-[10px] uppercase font-bold tracking-widest font-sans">Reviews</p><button onClick={() => setShowReviewModal({ open: true, goodId: g.id })} className="text-white/40 text-[9px] underline font-bold font-sans">후기 작성</button></div>
                    {g.reviews.map(r => (
                      <div key={r.id} className="bg-black/50 p-3 rounded-xl border border-white/5 font-sans">
                        <p className="text-white/80 text-[10px] font-bold font-sans">"{r.text}"</p>
                        <p className="text-[#daa520] text-[8px] mt-1 italic font-black font-sans">- {r.user} ({r.timestamp})</p>
                      </div>
                    ))}
                  </div>
                  <button className="w-full py-4 rounded-xl bg-white text-black text-sm border-2 border-[#daa520] font-black uppercase active:scale-95 shadow-md font-sans">BUY NOW</button>
                </div>
              ))}
            </div>
            <button onClick={() => setShowSellModal(true)} className="w-full py-5 rounded-2xl bg-[#daa520] text-black font-black text-lg border-2 border-white uppercase shadow-xl active:scale-95 italic font-sans">➕ 제국 시장에 상품 등록 ({COSTS_BEOM.SELL_ITEM} BEOM)</button>
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-black/95 border-2 border-[#daa520] p-1.5 rounded-2xl flex gap-3 z-[200] shadow-[0_0_50px_rgba(218,165,32,0.4)] backdrop-blur-xl animate-in slide-in-from-bottom-10 font-sans font-black">
        {['KEDHEON', 'CIVIL', 'NEXUS', 'VENDOR'].map(app => (
          <button key={app} className={`px-3 py-1.5 rounded-xl text-[9px] transition-all font-sans ${app === 'KEDHEON' ? 'bg-[#daa520] text-black' : 'text-white/40 hover:text-white'}`}>{app}</button>
        ))}
      </div>
      ...
      {showFanRoomModal && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-4 animate-in zoom-in-95 font-black font-sans">
          <div className="bg-[#111] p-8 rounded-[30px] border-4 border-[#daa520] w-full max-w-sm text-center shadow-2xl">
            ...
            <input type="text" value={fanRoomName} onChange={(e) => setFanRoomName(e.target.value)} placeholder="아지트 명칭" className="bg-black border-2 border-white p-4 rounded-xl w-full text-white text-center mb-6 outline-none font-black font-sans shadow-inner" />
            <div className="flex gap-2 font-sans font-black"><button onClick={() => setShowFanRoomModal(false)} className="flex-1 py-3 rounded-lg text-xs bg-white/10 border border-white/20 uppercase font-sans">Cancel</button><button onClick={createFanRoom} className="flex-1 py-3 rounded-lg text-xs bg-[#daa520] text-black border-2 border-white uppercase shadow-lg font-sans">Create</button></div>
          </div>
        </div>
      )}
      ...
      {showReviewModal.open && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-4 animate-in zoom-in-95 font-black font-sans">
          <div className="bg-[#111] p-8 rounded-[30px] border-4 border-white w-full max-w-sm text-center shadow-2xl">
            <h3 className="text-white text-xl mb-6 italic uppercase tracking-widest font-sans">Review</h3>
            ...
            <div className="flex gap-2 font-sans font-black"><button onClick={() => setShowReviewModal({ open: false, goodId: null })} className="flex-1 py-3 rounded-lg text-xs bg-white/10 border border-white/20 uppercase font-sans">Cancel</button><button onClick={postReview} className="flex-1 py-3 rounded-lg text-xs bg-[#daa520] text-black border-2 border-white uppercase shadow-lg font-sans">Post</button></div>
          </div>
        </div>
      )}
      ...
      {showSellModal && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-4 animate-in zoom-in-95 font-black font-sans">
          <div className="bg-[#111] p-8 rounded-[30px] border-4 border-[#daa520] w-full max-w-sm text-center shadow-2xl font-sans">
            <h3 className="text-[#daa520] text-xl mb-6 italic uppercase font-sans">Registration</h3>
            <div className="space-y-4 font-sans font-black"><input type="text" value={sellName} onChange={(e) => setSellName(e.target.value)} placeholder="상품명" className="bg-black border-2 border-white p-3 rounded-xl w-full text-white text-sm outline-none font-sans" /><input type="number" value={sellPrice} onChange={(e) => setSellPrice(Number(e.target.value))} placeholder="가격 (BEOM)" className="bg-black border-2 border-white p-3 rounded-xl w-full text-[#daa520] text-sm outline-none font-black font-sans" /><textarea value={sellDesc} onChange={(e) => setSellDesc(e.target.value)} placeholder="상세 설명" className="bg-black border-2 border-white p-3 rounded-xl w-full text-white/60 text-xs h-20 outline-none font-sans" /></div>
            <div className="flex gap-2 mt-6 font-sans font-black"><button onClick={() => setShowSellModal(false)} className="flex-1 py-3 rounded-lg text-xs bg-white/10 border border-white/20 uppercase font-sans">Cancel</button><button onClick={postGood} className="flex-1 py-3 rounded-lg text-xs bg-[#daa520] text-black border-2 border-white uppercase shadow-lg font-sans">Post Item</button></div>
          </div>
        </div>
      )}

      <div className="mt-24 opacity-30 text-center w-full pb-10 font-mono text-white text-[8px] tracking-[0.8em] uppercase font-black font-sans">KEDHEON EMPIRE | V61.6 ABSOLUTE MASTER | ohsangjo</div>
    </div>
  );
}
