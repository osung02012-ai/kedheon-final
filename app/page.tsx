...
const SectionTitle = ({ title, desc }: { title: string; desc: string }) => (
  <div className="flex flex-col items-center mb-14 gap-4 px-4">
    ...
    <p className="text-gray-400 font-bold text-2xl tracking-wide opacity-80 text-center max-w-5xl">
      {desc}
    </p>
  </div>
);
...
export default function KedheonPortal() {
  ...
  const t = translations[lang]; // 현재 언어 셋팅
  ...
  useEffect(() => {
    ...
    const savedLang = localStorage.getItem('k_lang') as 'KO' | 'EN';
  ...
  return (
    <div className="flex flex-col items-center bg-black min-h-screen text-white p-6 font-sans w-full overflow-x-hidden">
      ...
      <div className="w-full max-w-6xl flex justify-between items-center mt-6 mb-10">
        <button onClick={toggleLang} className="bg-[#daa520] text-black px-6 py-2 rounded-full font-black text-sm shadow-lg hover:scale-110 transition-all">
          {lang === 'KO' ? "ENGLISH MODE" : "한국어 모드"}
        </button>
        <div className="flex gap-4">
          <button onClick={() => {setTab('ROOKIE'); setViewMode('HUB');}} className={`px-10 py-3 rounded-2xl font-black text-lg ${tab === 'ROOKIE' ? 'bg-[#daa520] text-black' : 'bg-[#111] text-gray-500'}`}>ROOKIE</button>
          <button onClick={() => {setTab('PIONEER'); setViewMode('HUB');}} className={`px-10 py-3 rounded-2xl font-black text-lg ${tab === 'PIONEER' ? 'bg-[#daa520] text-black' : 'bg-[#111] text-gray-500'}`}>PIONEER</button>
        </div>
      </div>
              ...
              <>
                ...
                <div className="p-16 bg-[#111] rounded-[60px] border border-white/5 text-center">
                  ...
                  <div className="flex flex-wrap gap-5 justify-center mb-12">
                    <button onClick={() => {setCategory('ALL'); setViewMode('BOARD');}} className="px-12 py-6 rounded-3xl text-sm font-black bg-[#daa520] text-black shadow-lg">{t.enterAll}</button>
                    {['MUSIC', 'SPORTS', 'ANIME', 'DRAMA', 'MOVIE', 'ESPORTS', 'COMEDY'].map(cat => (
                    ...
                    {userTerritories.map(room => (
                      <button key={room.id} onClick={() => {setCategory(room.id); setViewMode('BOARD');}} className="px-12 py-6 rounded-3xl text-sm font-black bg-gradient-to-r from-[#daa520]/20 to-[#1a1a1a] text-[#daa520] border border-[#daa520]/30 shadow-xl font-bold tracking-widest">👑 {room.title}</button>
                    ))}
                  </div>
                  ...
                  <div className="relative w-full max-w-[650px] mx-auto aspect-[1.8/1] rounded-[50px] border border-white/10 overflow-hidden shadow-2xl group mb-10">
                    ...
                    <div className={`absolute inset-0 flex flex-col items-center justify-center p-12 transition-all ${isQrActive ? 'bg-transparent' : 'bg-black/95 backdrop-blur-3xl'}`}>
                      {isQrActive ? (
                        ...
                        <button onClick={() => {setBeomToken(beomToken-50); setIsQrActive(true);}} className="bg-[#daa520] text-black px-20 py-8 rounded-3xl font-black text-2xl shadow-xl hover:scale-105 transition-all">{t.qrAuth}</button>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-16">
                  {assets.filter(a => category === 'ALL' || a.category === category).map((a) => (
                    <div key={a.id} className="p-12 rounded-[70px] bg-[#111] border-l-[24px] border-gray-800 shadow-xl text-left">
                      <div className="flex justify-between mb-10"><h4 className="font-black text-white text-5xl tracking-tight leading-tight">{a.title}</h4><span className="text-gray-600 text-base font-mono font-black">{a.timestamp}</span></div>
                      {a.videoUrl && (
                      ...
                      <div className="flex justify-between items-center pt-10 border-t border-white/5">
                        <button onClick={() => setBeomToken(beomToken-100)} className="bg-[#daa520] text-black px-12 py-5 rounded-3xl font-black text-xl hover:scale-110 transition-all">{t.supportBtn}</button>
