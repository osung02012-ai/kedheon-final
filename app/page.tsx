...
export default function KedheonPortal() {
  ...
  const [goods] = useState<Good[]>([
    ...
    { id: 2, name: "KEDHEON CHARACTER T-SHIRT (SAMPLE)", price: 2500, img: "/kedheon-character.png", seller: "예사" }
  ]);
  ...
  const SectionHeader = ({ num, title }: { num: string; title: string }) => (
    <div className="w-full border-t-4 border-white pt-10 mb-8 text-left font-black">
      <h3 className="text-[#daa520] text-2xl md:text-4xl uppercase border-l-8 border-[#daa520] pl-4 leading-none italic tracking-tighter">
        {num}. 🌐 {title}
      </h3>
    </div>
  );
  ...
  useEffect(() => {
    setHasMounted(true);
    const saved = localStorage.getItem('kedheon_v59_master');
    if (saved) {
      try {
        const p = JSON.parse(saved);
        setBeomToken(p.token || 8141.88);
        if (Array.isArray(p.assets)) setAssets(p.assets);
      } catch (e) { console.error("Restore Failure"); }
    }
  }, []);

  useEffect(() => {
    if (hasMounted) {
      localStorage.setItem('kedheon_v59_master', JSON.stringify({ token: beomToken, assets }));
    }
  ...
  const postContent = () => {
    ...
    alert("피드 등록 성공 (10 BEOM)");
  };

  const supportPost = (id: number) => {
    if (tab === 'ROOKIE') return alert("시민권을 획득한 파이오니어만 찬양 가능합니다.");
    ...
    <div className="space-y-10">
      {assets.filter(a => category === 'ALL' || a.category === category).map(a => (
        <div key={a.id} className="bg-[#111] rounded-[40px] border-4 p-8 md:p-12 space-y-6 shadow-2xl relative transition-all border-white hover:border-[#daa520]">
          ...
          <p className="text-white text-xl md:text-2xl font-bold italic text-left leading-relaxed">"{a.desc}"</p>
      ...
      {assets.length === 0 && <p className="text-white/20 py-20 italic">현재 송출되는 방송이 없습니다.</p>}
    </div>
  );
      ...
      <div className="w-full max-w-5xl px-4 md:px-6">
        ...
        {tab === 'ROOKIE' ? (
          <div className="flex flex-col items-center py-16 animate-in fade-in duration-700">
            ...
            {!showOnboarding ? (
              <button onClick={() => setShowOnboarding(true)} className="bg-white text-black px-12 py-5 rounded-full text-xl md:text-2xl border-4 border-[#daa520] hover:scale-105 shadow-2xl mb-20">시민권 획득 가이드</button>
            ) : (
              <div className="w-full bg-[#111] p-6 md:p-10 rounded-[40px] border-4 border-white shadow-2xl space-y-8 animate-in slide-in-from-bottom-5 mb-20">
                ...
                <div className="flex gap-4 font-sans"><button onClick={() => window.open(PI_APP_STORE_URL)} className="flex-1 bg-white text-black py-4 rounded-xl border-4 border-[#daa520]">App Store</button><button onClick={() => window.open(PI_PLAY_STORE_URL)} className="flex-1 bg-white text-black py-4 rounded-xl border-4 border-[#daa520]">Play Store</button></div>
              </div>
            )}
            <SectionHeader num="00" title="EMPIRE LIVE FEED (VIEW ONLY)" />
          ...
          <div className="flex flex-col gap-12 py-10 animate-in slide-in-from-bottom-5">
            ...
            <div className="w-full bg-[#111] p-8 md:p-12 rounded-[50px] border-4 border-[#daa520] shadow-2xl">
              <h2 className="text-[#daa520] text-3xl md:text-6xl uppercase italic mb-6">누리고 즐기고 선점하세요</h2>
            ...
            <div className="flex flex-col w-full">
              ...
              <div className="bg-[#111] p-6 md:p-10 rounded-[40px] border-4 border-white flex flex-col items-center gap-8 shadow-2xl">
                ...
                <button onClick={() => { setIsQrActive(true); setBeomToken(p => p - 50); }} className="bg-[#daa520] text-black px-12 py-5 rounded-2xl text-xl md:text-3xl border-4 border-white shadow-2xl active:scale-95 transition-all uppercase font-black">인증 활성화 (50 BEOM)</button>
              </div>
            </div>
            ...
            <div className="flex flex-col w-full text-left">
              ...
              <div className="w-full bg-[#111] p-6 md:p-10 rounded-[40px] border-4 border-[#daa520] space-y-6 mb-12 shadow-xl">
                ...
                <input type="text" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder={`${postType === 'BROADCAST' ? '방송' : '창작물'} 제목`} className="bg-black border-4 border-white p-4 rounded-xl w-full text-lg md:text-2xl text-white outline-none focus:border-[#daa520] font-black" />
                <textarea value={newDesc} onChange={(e) => setNewDesc(e.target.value)} placeholder="상세 내용을 입력하십시오" className="bg-black border-4 border-white p-4 rounded-xl w-full text-sm md:text-lg text-white/80 h-32 resize-none outline-none focus:border-[#daa520] font-bold" />
                <button onClick={postContent} className="w-full py-5 rounded-2xl bg-[#daa520] text-black text-xl md:text-2xl border-4 border-white shadow-lg active:scale-95 uppercase font-black">제국 피드 등록하기 (10 BEOM)</button>
              </div>
            ...
            <div className="flex flex-col w-full text-left">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                {goods.map(g => (
                  <div key={g.id} className="bg-[#111] rounded-[40px] border-4 border-white p-6 shadow-2xl flex flex-col items-center gap-4 relative">
                    ...
                    <button onClick={() => alert("현재 예시 상품입니다. 정식 출시를 기다려주세요!")} className="w-full py-4 rounded-2xl bg-white text-black text-xl border-4 border-[#daa520] hover:bg-[#daa520] transition-all uppercase font-black">COMING SOON</button>
                  </div>
                ))}
              </div>
              ...
              <button onClick={() => setShowCreateModal(true)} className="w-full py-6 mt-8 rounded-3xl bg-[#daa520] text-black font-black text-2xl border-4 border-white shadow-xl hover:scale-105 transition-all uppercase italic">➕ 새로운 팬방 개설 (500 BEOM)</button>
            </div>
          </div>
        )}
      </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/98 backdrop-blur-3xl z-[300] flex items-center justify-center p-6 animate-in zoom-in-95">
          <div className="bg-[#111] p-10 rounded-[50px] border-8 border-[#daa520] w-full max-w-2xl text-center shadow-2xl font-black">
            <h3 className="text-[#daa520] text-4xl mb-10 italic uppercase font-sans">New Fan Territory</h3>
            ...
            <div className="flex gap-4 font-sans"><button onClick={() => setShowCreateModal(false)} className="flex-1 py-5 rounded-2xl font-black text-xl bg-white/10 border-4 border-white uppercase">Cancel</button><button onClick={() => { setShowCreateModal(false); setCreateTitle(''); alert("개설 완료"); }} className="flex-1 py-5 rounded-2xl font-black text-xl bg-[#daa520] text-black border-4 border-white shadow-xl uppercase transition-all">Create</button></div>
          </div>
        </div>
      )}

      <div className="mt-20 opacity-40 text-center w-full pb-10 font-mono text-white text-[10px] tracking-[1em] uppercase font-black">KEDHEON EMPIRE | INTEGRITY MASTER V59.0 | ohsangjo</div>
    </div>
  );
}
